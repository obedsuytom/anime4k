...
cursor.execute("DROP TABLE IF EXISTS anime_info")
cursor.execute("""
...
async def load_anime_info(anime_name, bot):
    """
    Берём данные из anime_info или подгружаем с Shikimori и Anilist.
    Кэшируем постеры через Telegram file_id.
    """
    # Проверяем базу
    cursor.execute("SELECT poster, poster_file_id, score, genres, year FROM anime_info WHERE anime=?", (anime_name,))
    row = cursor.fetchone()
    ...
    if row:
        ...
        if not poster_file_id and poster_url:
            try:
                msg = await bot.send_photo(chat_id=bot.id, photo=poster_url)  # отправляем в бота, можно в отдельный чат для кэширования
...
async def inline_search(query: types.InlineQuery):
    ...
    if search_text == "all":
        ...
        cursor.execute("SELECT DISTINCT anime FROM videos WHERE LOWER(anime) LIKE ? ORDER BY anime", (f"%{search_text}%",))
        all_animes = [row[0] for row in cursor.fetchall()]
    ...
    for anime_name in page_animes:
        data = await load_anime_info(anime_name, bot)
        if not data:
            continue
        ...
        thumb = poster_file_id if poster_file_id else poster_url or "https://via.placeholder.com/150"
        ...
        results.append(
            InlineQueryResultArticle(
                ...
                thumb_url=thumb if poster_file_id is None else None,
                thumb_width=150,
                thumb_height=150
            )
        )
