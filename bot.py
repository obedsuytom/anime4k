...
async def show_picker(target, state, page=0):
    data = await state.get_data()
...
async def show_owned_collections(call: types.CallbackQuery, mode: str, page: int = 0):
    ...
    for collection_id, title, status in rows:
        ...
        buttons.append([
            InlineKeyboardButton(
                text=f"{status_icon} {button_title}",
...
async def edit_collection_open(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_data({
        ...
        "photo": photo,
        "selected": get_collection_animes(collection_id),
...
async def edit_collection_field(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await call.message.answer(prompts[field], reply_markup=edit_input_back_kb)
    await call.answer()
...
async def edit_collection_title(message: types.Message, state: FSMContext):
    ...
    if not title:
        await message.answer("Название не может быть пустым", reply_markup=edit_input_back_kb)
        return
    if len(title) > 100:
        await message.answer("Название должно быть не длиннее 100 символов", reply_markup=edit_input_back_kb)
        return
...
async def edit_collection_description(message: types.Message, state: FSMContext):
    ...
    if len(description) > 500:
        await message.answer("Описание должно быть не длиннее 500 символов", reply_markup=edit_input_back_kb)
        return
...
async def edit_collection_photo(message: types.Message, state: FSMContext):
    if not message.photo:
        await message.answer("Отправь именно фото", reply_markup=edit_input_back_kb)
        return
