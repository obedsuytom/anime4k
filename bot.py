...
async def pay_rub_handler(call: types.CallbackQuery):
    ...
    label, pay_url = create_yoomoney_payment_link(user_id, rub_amount, period_key)
    if not label or not pay_url:
        await call.message.answer(
            "❌ Не удалось создать ссылку оплаты ЮMoney. Проверь YOOMONEY_RECEIVER в переменных сервера."
        )
        await call.answer()
        return

    text = (
        f"<tg-emoji emoji-id=\"5350452584119279096\">👍</tg-emoji> <b>Оплата подписки рублями через ЮMoney</b>\n\n"
        f"Сумма: <b>{rub_amount}₽</b>\n"
        f"Тариф: <b>{period_key.replace('_', ' ')}</b>\n\n"
        "Нажмите кнопку ниже и оплатите картой или кошельком ЮMoney.\n"
        "После успешной оплаты подписка активируется автоматически через HTTP-уведомление ЮMoney.\n\n"
        "Если подписка не активировалась в течение пары минут — проверь, что в ЮMoney включены HTTP-уведомления на <code>/yoomoney</code>."
    )
