...
async def process_multi_range(
    ...
    if end_ep < start_ep:
        await message.answer(
            "❌ Конечная серия не может быть меньше начальной."
        )
        return
    ...
    dub_hash = data["dub_hash"]

    cursor.execute(
        "SELECT anime, dub, season FROM videos"
    )
    ...
    for ep, file_id in episodes:
        try:
            ...
            print(f"Ошибка отправки серии {ep}: {e}")
...
async def process_multi_range(
    ...
    for ep, file_id in episodes:
        try:
            ...
            # защита от флудлимитов Telegram
            await asyncio.sleep(0.5)

        except Exception as e:
            print(
                f"[MULTI_EPISODES] Ошибка отправки серии "
                f"{ep}: {e}"
            )

    await message.answer(
        f"✅ Отправлено серий: {sent_count}"
    )
