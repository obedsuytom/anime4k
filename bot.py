...
async def load_anime_info(anime_name):
    """Берём данные из базы anime_info или подгружаем через API и сохраняем"""
...
async def inline_search(query):
    ...
    for anime_name in page_animes:
        ...
        # В inline результатах видны постер и описание
        ...
        results.append(
            InlineQueryResultArticle(
                id=str(uuid4()),
                title=anime_name,
                description=description_text,  # отображается в поиске
                input_message_content=InputTextMessageContent(
                    message_text=anime_name  # при нажатии в чат вставляется только название
                ),
