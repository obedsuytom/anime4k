...
BOOKMARK_FAVORITE_EMOJI_ID = "5438496463044752972"

BOOKMARK_LIST_TITLES = {
    "favorite": f'<tg-emoji emoji-id="{BOOKMARK_FAVORITE_EMOJI_ID}">👍</tg-emoji> Избранное',
    "watching": f'<tg-emoji emoji-id="{STATUS_BUTTONS["watching"]["icon_custom_emoji_id"]}">👍</tg-emoji> Смотрю',
    "completed": f'<tg-emoji emoji-id="{STATUS_BUTTONS["completed"]["icon_custom_emoji_id"]}">👍</tg-emoji> Просмотрено',
    "planned": f'<tg-emoji emoji-id="{STATUS_BUTTONS["planned"]["icon_custom_emoji_id"]}">👍</tg-emoji> В планах',
    "dropped": f'<tg-emoji emoji-id="{STATUS_BUTTONS["dropped"]["icon_custom_emoji_id"]}">👍</tg-emoji> Брошено',
}
...
async def show_bookmark_list(call, status, page=0):
    ...
    for anime in items:
        # Переход в карточку аниме делаем так же, как в топе рейтинга: через anime_index|idx.
        try:
            cb = f"anime_index|{all_animes.index(anime)}"
        except ValueError:
            anime_id = get_or_create_anime_id(anime)
            cb = f"anime_bookmark|{anime_id}"

        builder.row(InlineKeyboardButton(
            text=cut_title(format_anime_title(anime)),
            callback_data=cb,
            icon_custom_emoji_id=BOOKMARK_FAVORITE_EMOJI_ID if status == "favorite" else STATUS_BUTTONS.get(status, STATUS_BUTTONS[None])["icon_custom_emoji_id"],
        ))
...
async def bookmarks_menu(call: types.CallbackQuery):
    kb=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=" Избранное", icon_custom_emoji_id=BOOKMARK_FAVORITE_EMOJI_ID, callback_data="bookmark_list|favorite|0")],
        [InlineKeyboardButton(text=" Смотрю", icon_custom_emoji_id=STATUS_BUTTONS["watching"]["icon_custom_emoji_id"], callback_data="bookmark_list|watching|0")],
        [InlineKeyboardButton(text=" В планах", icon_custom_emoji_id=STATUS_BUTTONS["planned"]["icon_custom_emoji_id"], callback_data="bookmark_list|planned|0")],
        [InlineKeyboardButton(text=" Просмотрено", style="success", icon_custom_emoji_id=STATUS_BUTTONS["completed"]["icon_custom_emoji_id"], callback_data="bookmark_list|completed|0")],
        [InlineKeyboardButton(text=" Брошено", style="danger", icon_custom_emoji_id=STATUS_BUTTONS["dropped"]["icon_custom_emoji_id"], callback_data="bookmark_list|dropped|0")],
        [InlineKeyboardButton(text=" Назад", style="primary", icon_custom_emoji_id="5352759161945867747", callback_data="profile_menu")]
    ])
    await call.message.edit_text("<tg-emoji emoji-id=\"5222444124698853913\">👍</tg-emoji> Ваши закладки:", parse_mode="HTML", reply_markup=kb)
    await call.answer()
