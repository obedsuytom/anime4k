...
async def send_video(call: types.CallbackQuery):
    ...
    if prev_exists:
        kb.add(InlineKeyboardButton("⬅️ Предыдущая серия", callback_data=f"ep|{make_cb_id(anime, dub, str(season), str(ep - 1))}|{page}"))
    if next_exists:
        kb.add(InlineKeyboardButton("➡️ Следующая серия", callback_data=f"ep|{make_cb_id(anime, dub, str(season), str(ep + 1))}|{page}"))

    # Кнопка для облака (если ссылка)
    if isinstance(file_id, str) and file_id.startswith("http"):
        kb.add(InlineKeyboardButton("🌐 Скачать серию из облака", url=file_id))

    # Возврат к озвучкам и меню
    kb.add(
    ...
    # Отправка: если облако — просто текст, если file_id — видео
    if isinstance(file_id, str) and file_id.startswith("http"):
