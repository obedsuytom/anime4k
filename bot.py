...
async def anime_selected(call: types.CallbackQuery):
    ...
    current_index = STATUS_CYCLE.index(current) if current in STATUS_CYCLE else 0
    next_status = STATUS_CYCLE[(current_index + 1) % len(STATUS_CYCLE)]
    anime_id = get_or_create_anime_id(anime)

    status_text = "👀 Не смотрю" if next_status is None else BOOKMARK_NAMES[next_status]
...
async def choose_dub_after_season(call: types.CallbackQuery):
    ...
    builder.row(
        InlineKeyboardButton(
            text="👀 Не смотрю" if next_status is None else BOOKMARK_NAMES[next_status],
            callback_data=f"set_status|{anime_id}|{next_status or 'none'}",
            style="primary",
            icon_custom_emoji_id="5350658016700013471"
        ),
...
async def refresh_anime_buttons(call: types.CallbackQuery, anime: str):
    """Меняет только текст существующих кнопок, сохраняя оформление."""
    statuses = get_bookmark_status(call.from_user.id, anime)
    current = next((s for s in BOOKMARK_STATUSES if s in statuses), None)

    # цикл: Не смотрю -> Смотрю -> Просмотрено -> В планах -> Брошено -> Не смотрю
    current_index = STATUS_CYCLE.index(current) if current in STATUS_CYCLE else 0
    next_status = STATUS_CYCLE[(current_index + 1) % len(STATUS_CYCLE)]

    status_text = "👀 Не смотрю" if next_status is None else BOOKMARK_NAMES[next_status]
    favorite_text = "⭐ В избранном" if "favorite" in statuses else "⭐ Избранное"

    markup = call.message.reply_markup

    if markup and markup.inline_keyboard and len(markup.inline_keyboard) > 0:
        # Переписываем только текст первых двух кнопок.
        # Все остальные поля (цвет, custom emoji, callback_data) остаются прежними.
        first_row = markup.inline_keyboard[0]

        if len(first_row) >= 1:
            first_row[0] = first_row[0].model_copy(update={"text": status_text})

        if len(first_row) >= 2:
            first_row[1] = first_row[1].model_copy(update={"text": favorite_text})

        await call.message.edit_reply_markup(reply_markup=markup)
        return

    # запасной вариант, если у сообщения нет старой клавиатуры
    anime_id = get_or_create_anime_id(anime)
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(
