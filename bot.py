...
async def referral_menu(call: types.CallbackQuery):
    ...
    text = (
        ...
        "📌 <b>Условия:</b>\n"
        "• Подписка 30+ дней — оба получают 1 неделю 🎁\n"
        "• За каждого 5-го реферала вы получите 1 месяц 🔥\n\n"
    )
    ...
    if invited_users:
        ...
        for (uid,) in invited_users:
            cursor.execute(
                "SELECT first_name FROM referrals WHERE user_id = ?",
                (uid,)
            )
            user_row = cursor.fetchone()

            name = user_row[0] if user_row and user_row[0] else "Пользователь"

            # 🔗 правильная ссылка на профиль
        ...
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    ...
                    callback_data="choose_plan",
                    style="success"  # 🟢 зелёная (логично для покупки)
                )
            ],
            [
                InlineKeyboardButton(
...
async def register_user(call: types.CallbackQuery):
    ...
    await send_and_track(
        ...
        "👥 Твой реферальный код уже создан!\n"
        "Зайди в раздел «Приведи друга» чтобы посмотреть его.\n\n"
        "Приятного просмотра 🍿",
        reply_markup=kb
    )
...
async def pay_crypto_handler(call: types.CallbackQuery):
    ...
    text = (
        ...
        "нажмите кнопку <b>✅ Подтвердить оплату</b>.\n\n"
...
async def handle_crypto_webhook(request):
    try:
        ...
        if days is None:
            await bot.send_message(
                user_id,
                "🎉 Ваша подписка НАВСЕГДА активирована!"
            )
        else:
            await bot.send_message(
                user_id,
...
async def handle_check(message: types.Message):
    ...
    builder = InlineKeyboardBuilder()
    builder.add(InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="back_menu"))
    kb = builder.as_markup()
...
async def live_search(message: types.Message):
    ...
    for anime in found_animes:
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="▶️ Открыть", callback_data=f"anime_index|{anime_idx}"))
        builder.row(InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="back_menu"))
        kb = builder.as_markup()
...
async def choose_dub_after_season(call: types.CallbackQuery):
    ...
    for dub in dubs:
        ...
        builder.row(InlineKeyboardButton(text=f"▶️ {dub}", callback_data=f"dub|{cb_id}"))
    ...
    builder.row(InlineKeyboardButton(text="⬅️ К аниме", callback_data=f"anime_index|{anime_idx}"))
    builder.row(InlineKeyboardButton(text="🏠 Меню", callback_data="back_menu"))
...
async def show_episodes_after_dub(call: types.CallbackQuery):
    ...
    nav_buttons = []
    if start > 0:
        nav_buttons.append(InlineKeyboardButton(text="◀️", callback_data=f"dub|{dub_hash}|{page-1}"))
    if end < len(episodes):
        nav_buttons.append(InlineKeyboardButton(text="▶️", callback_data=f"dub|{dub_hash}|{page+1}"))
    if nav_buttons:
        builder.row(*nav_buttons)
    ...
    builder.row(InlineKeyboardButton(text="📂 Быстрый переход", callback_data=f"ranges|{dub_hash}"))
