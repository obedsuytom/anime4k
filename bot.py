...
ADMIN_BADGE_HTML = "".join(
    f'<tg-emoji emoji-id="{emoji_id}">👍</tg-emoji>'
    for emoji_id in ADMIN_BADGE_EMOJI_IDS
)
PROFILE_COMMENTS_PER_PAGE = 5
PROFILE_COMMENT_MAX_LENGTH = 500
MAINTENANCE_MODE = False
MAINTENANCE_TEXT = "бот закрыт на техработы связанные с обновлением просим вас подождать"
...
def set_maintenance_mode(enabled: bool):
    """Переключает режим технических работ и сохраняет его в БД."""
...
async def stop_bot_for_maintenance(message: types.Message):
    if not message.from_user or message.from_user.id not in ADMINS:
        await message.answer(MAINTENANCE_TEXT if MAINTENANCE_MODE else "❌ Нет доступа.")
        return

    if MAINTENANCE_MODE:
        await message.answer("🛠 Режим технических работ уже включён.")
        return

    set_maintenance_mode(True)
    await message.answer(
        "🛠 Бот закрыт для пользователей и доступен только администраторам.\n\n"
        "Чтобы снова открыть бота, используй /resume"
    )


@router.message(Command("resume"))
async def resume_bot_after_maintenance(message: types.Message):
    if not message.from_user or message.from_user.id not in ADMINS:
        await message.answer(MAINTENANCE_TEXT if MAINTENANCE_MODE else "❌ Нет доступа.")
        return

    if not MAINTENANCE_MODE:
        await message.answer("✅ Бот уже открыт для пользователей.")
        return

    set_maintenance_mode(False)
    await message.answer("✅ Технические работы завершены. Бот снова открыт для всех.")


# Зарегистрированы до остальных обработчиков: не-администраторы не проходят дальше.
@router.message(
    lambda message: (
        MAINTENANCE_MODE
...
async def maintenance_message_blocker(message: types.Message):
    await message.answer(MAINTENANCE_TEXT)


@router.callback_query(
    lambda call: MAINTENANCE_MODE and call.from_user.id not in ADMINS
)
async def maintenance_callback_blocker(call: types.CallbackQuery):
    await call.answer(MAINTENANCE_TEXT, show_alert=True)
...
cursor.execute("SELECT value FROM bot_settings WHERE key='maintenance_mode' LIMIT 1")
maintenance_row = cursor.fetchone()
MAINTENANCE_MODE = bool(maintenance_row and maintenance_row[0] == "1")
...
profile_comment_columns = [column[1] for column in cursor.fetchall()]
if "author_name_html" not in profile_comment_columns:
    cursor.execute("ALTER TABLE profile_comments ADD COLUMN author_name_html TEXT")
    db.commit()
if "moderation_status" not in profile_comment_columns:
    cursor.execute(
...
ON profile_comments(profile_user_id, id DESC)
""")
db.commit()
...
ON profile_moderation(status, id DESC)
""")
db.commit()
...
class ProfileCommentState(StatesGroup):
    waiting_text = State()
...
async def get_anime_info(title: str):
    ...
    info = {
        ...
        "studio": ", ".join(
            str(studio.get("name") or studio.get("filtered_name") or "").strip()
            for studio in (full.get("studios") or [])
            if isinstance(studio, dict)
            and (studio.get("name") or studio.get("filtered_name"))
        ) or "—",
...
# Предполагается, что эти объекты уже существуют у тебя:
# bot
# ADMINS
# db_path = "anime.db"
#
...
DURATION_SCANS: set[int] = set()


def get_db_connection() -> sqlite3.Connection:
    """
    Отдельное соединение SQLite для duration_scan.
    Это важно: не используем глобальный cursor во время долгих async-операций.
    """
...
async def detect_video_duration(
    chat_id: int,
    file_id: str,
) -> int | None:
    """
    Временно отправляет Telegram-видео, получает duration
    и удаляет временное сообщение.

    Важные изменения:
    - timeout на send_video;
    - timeout на delete_message;
    - TelegramRetryAfter пробрасывается наружу;
    - все прочие ошибки тоже пробрасываются наружу.
    """
    temporary_message: types.Message | None = None

    try:
        logging.info(
            "duration: отправка video file_id=%s",
            file_id,
        )

        temporary_message = await asyncio.wait_for(
            bot.send_video(
                chat_id=chat_id,
                video=file_id,
        ...
        if (
            temporary_message.video
            and temporary_message.video.duration
            and temporary_message.video.duration > 0
        ):
            duration = int(temporary_message.video.duration)

        logging.info(
            "duration: получено file_id=%s duration=%s message_id=%s",
            file_id,
            duration,
        ...
        logging.warning(
            "duration: timeout при send_video file_id=%s",
            file_id,
        )
        raise
        ...
        if temporary_message is not None:
            try:
                logging.info(
                    "duration: удаление временного сообщения id=%s",
                    temporary_message.message_id,
                )
                ...
                logging.exception(
                    "duration: TelegramRetryAfter при удалении message_id=%s",
                    temporary_message.message_id,
                )

            except asyncio.TimeoutError:
                logging.warning(
                    "duration: timeout при удалении message_id=%s",
                    temporary_message.message_id,
                )

            except Exception:
                logging.exception(
                    "duration: ошибка удаления message_id=%s",
                    temporary_message.message_id,
                )


async def detect_video_duration_with_retry(
    chat_id: int,
    file_id: str,
    max_attempts: int = 4,
) -> int | None:
    """
    Получает duration с повторными попытками.

    Обрабатывает:
    - TelegramRetryAfter / 429;
    - timeout;
    - временные ошибки Telegram.
    """
    last_error: Exception | None = None

    for attempt in range(1, max_attempts + 1):
        try:
            return await detect_video_duration(
                chat_id=chat_id,
                file_id=file_id,
            )
            ...
            logging.warning(
                "duration: TelegramRetryAfter "
                "file_id=%s attempt=%s/%s wait=%.1fs",
                file_id,
                attempt,
                max_attempts,
                retry_after,
    ...
    if last_error is not None:
        logging.warning(
            "duration: окончательно не удалось получить duration "
            "file_id=%s error=%r",
            file_id,
            last_error,
        )
...
def get_duration_scan_files() -> tuple[list[tuple[str, int]], int]:
    """
    Получает список уникальных Telegram file_id и количество связанных записей.

    Работает в отдельном SQLite connection.
    """
    ...
    try:
        ...
        return files_to_scan, link_rows

    finally:
        conn.close()


def update_duration_in_db(
    conn: sqlite3.Connection,
    file_id: str,
    duration: int,
) -> int:
    """
    Обновляет duration для всех записей с данным file_id,
    где duration ещё не заполнен.

    Есть retry на случай временного database lock.
    """
    for attempt in range(1, 6):
        try:
            ...
            logging.warning(
                "SQLite locked при update file_id=%s "
                "attempt=%s/5",
                file_id,
                attempt,
            )
            ...
            time_to_wait = min(attempt * 2, 10)
            # SQLite вызов синхронный, поэтому даём event loop отдохнуть.
            # Здесь функция синхронная — sleep будет выполнен вызывающим кодом.
            import time
...
async def duration_scan(message: types.Message):
    """
    Заполняет duration уже сохранённых Telegram-видео.

    Основные улучшения:
    - отдельное SQLite-соединение;
    - WAL;
    - timeout на Telegram;
    - обработка TelegramRetryAfter;
    - commit каждые 25 файлов;
    - периодическое обновление прогресса;
    - при остановке исключением соединение корректно закрывается;
    - один зависший Telegram-запрос не может зависнуть навсегда.
    """
    user_id = message.from_user.id
    ...
    if user_id in DURATION_SCANS:
        await message.answer(
            "⏳ Сканирование хронометража уже запущено."
        )
        return

    # Получаем список до начала долгой работы.
    try:
        files_to_scan, link_rows = await asyncio.to_thread(
            get_duration_scan_files
        )
    except Exception:
        logging.exception(
            "duration_scan: ошибка получения списка файлов"
        )

        await message.answer(
            "❌ Не удалось прочитать список видео из базы."
        )
        return

    if not files_to_scan:
        text = (
            "✅ У всех Telegram-видео хронометраж уже заполнен."
        )

        if link_rows:
            text += (
                f"\n\n⚠️ Ссылки без хронометража: "
                f"{link_rows}. "
                "Они не сканируются через Telegram file_id."
            )
    ...
    total_rows = sum(
        int(row_count)
        for _, row_count in files_to_scan
    )
    ...
    # Отдельное соединение только для этого сканирования.
    db = get_db_connection()

    # Частота commit.
    COMMIT_EVERY = 25

    # Частота изменения сообщения прогресса.
    PROGRESS_EVERY = 10

    # Небольшая задержка между Telegram запросами.
    REQUEST_DELAY = 0.25

    try:
        for index, (file_id, affected_rows) in enumerate(
            files_to_scan,
            start=1,
        ):
            file_id = str(file_id)
            ...
            logging.info(
                "duration_scan: [%s/%s] START file_id=%s",
                index,
                total_files,
                file_id,
            )
            ...
            try:
                duration = await detect_video_duration_with_retry(
                    chat_id=message.chat.id,
                    file_id=file_id,
                    max_attempts=4,
                )
                ...
                logging.exception(
                    "duration_scan: "
                    "неожиданная ошибка Telegram file_id=%s",
                    file_id,
                )
            ...
            if duration is not None and duration > 0:
                try:
                    ...
                    filled_files += 1
                    filled_rows += updated_rows

                    logging.info(
                        "duration_scan: [%s/%s] "
                        "duration=%s updated_rows=%s",
                        index,
                        total_files,
                        duration,
                        updated_rows,
                ...
                logging.warning(
                    "duration_scan: duration не получен "
                    "[%s/%s] file_id=%s",
                    index,
                    total_files,
                    file_id,
            ...
            if (
                ...
                try:
                    await progress_message.edit_text(
                        "⏱ <b>Сканирование хронометража</b>\n\n"
                        f"Обработано: "
                        f"<b>{index}/{total_files}</b>\n"
                        f"Заполнено видео: "
                        f"<b>{filled_files}</b>\n"
                        f"Заполнено записей: "
                        f"<b>{filled_rows}</b>\n"
                        f"Ошибок: "
                        f"<b>{len(failed_files)}</b>",
                        parse_mode="HTML",
                    )
                    ...
                    logging.warning(
                        "duration_scan: "
                        "429 при обновлении прогресса, "
                        "wait=%s",
                        error.retry_after,
                    )

                except Exception:
                    # Ошибка редактирования прогресса "
                    # не должна останавливать сканирование.
                    logging.exception(
                        "duration_scan: "
                        "не удалось обновить progress_message"
                    )

            # Не бомбим Telegram слишком быстро.
        ...
        result = (
            ...
            f"Проверено уникальных видео: "
            f"<b>{total_files}</b>\n"
            f"Хронометраж заполнен у видео: "
            f"<b>{filled_files}</b>\n"
            f"Обновлено записей серий: "
            f"<b>{filled_rows}</b>\n"
        ...
        try:
            ...
            logging.exception(
                "duration_scan: "
                "не удалось отправить финальный результат"
            )

    except asyncio.CancelledError:
        # Если задача была остановлена.
        logging.warning(
            "duration_scan: задача отменена "
            "на %s/%s",
            filled_files,
            total_files,
        )
        ...
        try:
            ...
            logging.exception(
                "duration_scan: commit после CancelledError"
            )
        ...
        logging.exception(
            "duration_scan: критическая ошибка"
        )
        ...
        try:
            ...
            logging.exception(
                "duration_scan: rollback failed"
            )

        try:
            await progress_message.edit_text(
                "❌ <b>Сканирование остановлено из-за ошибки.</b>\n\n"
                f"Обработано: <b>{filled_files}/{total_files}</b>\n"
        ...
        try:
            ...
            logging.exception(
                "duration_scan: ошибка закрытия SQLite"
            )
...
async def my_collection_open(call: types.CallbackQuery):
    ...
    caption += f"Статус: <b>{COLLECTION_STATUS_NAMES.get(status, status)}</b>"
...
async def collection_author_profile(call: types.CallbackQuery):
    ...
    comments_total = get_profile_comments_count(owner_id)
    ...
    text += (
        ...
        f'<tg-emoji emoji-id="5440621591387980068">👍</tg-emoji> Просмотрено часов: <b>{completed_hours}</b>\n\n'
        f"Автор подборки: <b>{html.escape(str(collection_title))}</b>"
    )

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f" Комментарии ({comments_total})",
            callback_data=f"profile_comments|{owner_id}|0|{collection_id}",
...
async def broadcast(message: types.Message):
    ...
    async def send_task():
        ...
        if target == "all":
            ...
            report_text = (
                "✅ Рассылка завершена\n\n"
                f"Отправлено пользователям: {total}\n"
                f"Дошло: {sent}\n"
                f"Не дошло: {errors}"
            )
...
async def show_profile_edit_menu(target, state: FSMContext, notice: str = ""):
    ...
    text += (
        ...
        f"Анимированные эмодзи в нике: {nickname_emoji_access}."
    )
...
async def account_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    comments_total = get_profile_comments_count(user_id)
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f" Комментарии ({comments_total})",
            callback_data=f"profile_comments|{user_id}|0|0",
            icon_custom_emoji_id="5443038326535759644"
        )],
        [InlineKeyboardButton(
...
def unpack_profile_media(value: str | None):
    if not value:
        return None, None
    value = str(value)
    if value.startswith("animation:"):
        return "animation", value.split(":", 1)[1]
    return "photo", value
...
async def send_profile_card(
    ...
    media_type, file_id = unpack_profile_media(media_value)
    if media_type == "animation" and file_id:
        return await send_and_track(
            user_id,
            source_message.answer_animation,
            animation=file_id,
            caption=text,
...
def get_profile_comments_count(profile_user_id: int) -> int:
    cursor.execute(
        """
        SELECT COUNT(*)
...
async def show_profile_comments(
    ...
    total = get_profile_comments_count(profile_user_id)
    total_pages = max(1, (total + PROFILE_COMMENTS_PER_PAGE - 1) // PROFILE_COMMENTS_PER_PAGE)
    page = min(max(0, int(page)), total_pages - 1)
    offset = page * PROFILE_COMMENTS_PER_PAGE

    cursor.execute(
        """
        SELECT author_name, author_name_html, comment_text
        FROM profile_comments
        WHERE profile_user_id=?
          AND COALESCE(moderation_status, 'approved') != 'rejected'
        ORDER BY id DESC
        LIMIT ? OFFSET ?
        """,
        (profile_user_id, PROFILE_COMMENTS_PER_PAGE, offset)
    )
    rows = cursor.fetchall()

    text_parts = [
        "<b>Комментарии к профилю</b>",
        f"Всего комментариев: <b>{total}</b>",
    ]
    if rows:
        for author_name, author_name_html, comment_text in rows:
            rendered_author = (
                str(author_name_html)
                if author_name_html
                else html.escape(str(author_name))
            )
            text_parts.append(
                f"<b>{rendered_author}</b>\n"
                f"{html.escape(str(comment_text))}"
            )
    else:
        text_parts.append("Комментариев пока нет.")
    ...
    if page > 0:
        navigation.append(InlineKeyboardButton(
            text="⬅️",
            callback_data=f"profile_comments|{profile_user_id}|{page - 1}|{collection_id}"
        ))
    ...
    if call.from_user.id != profile_user_id:
        buttons.append([InlineKeyboardButton(
            text=" Оставить комментарий",
            callback_data=f"profile_comment_add|{profile_user_id}|{collection_id}",
            style="success",
            icon_custom_emoji_id="5373251851074415873"
        )])

    back_callback = (
        f"collection_author_profile|{collection_id}"
        if collection_id
        else "account_menu"
    )
    buttons.append([InlineKeyboardButton(
    ...
    try:
        ...
        pass
    comments_text = "\n\n".join(text_parts)
    comments_markup = InlineKeyboardMarkup(inline_keyboard=buttons)
    try:
        await send_and_track(
            call.from_user.id,
            call.message.answer,
            comments_text,
            parse_mode="HTML",
            reply_markup=comments_markup
        )
    except Exception as error:
        if "Invalid custom emoji identifier" not in str(error):
            raise
        safe_markup = InlineKeyboardMarkup(inline_keyboard=[
        ...
        await send_and_track(
            call.from_user.id,
            call.message.answer,
            profile_html_to_plain(comments_text),
            parse_mode=None,
...
@router.callback_query(F.data.startswith("profile_comments|"))
async def profile_comments_page(call: types.CallbackQuery):
    try:
        _, profile_raw, page_raw, collection_raw = call.data.split("|", 3)
        profile_user_id = int(profile_raw)
        page = int(page_raw)
        collection_id = int(collection_raw)
    except (TypeError, ValueError):
        await call.answer("Некорректные данные", show_alert=True)
        return
    await show_profile_comments(call, profile_user_id, page, collection_id)


@router.callback_query(F.data.startswith("profile_comment_add|"))
async def profile_comment_add(call: types.CallbackQuery, state: FSMContext):
    try:
        _, profile_raw, collection_raw = call.data.split("|", 2)
        profile_user_id = int(profile_raw)
        collection_id = int(collection_raw)
    except (TypeError, ValueError):
        await call.answer("Некорректные данные", show_alert=True)
    ...
    if call.from_user.id == profile_user_id:
        await call.answer("Нельзя оставлять комментарий самому себе.", show_alert=True)
        return

    await state.set_state(ProfileCommentState.waiting_text)
    await state.set_data({
        "comment_profile_user_id": profile_user_id,
        "comment_collection_id": collection_id,
    })
    ...
    await send_and_track(
        call.from_user.id,
        call.message.answer,
        f"Напишите комментарий к профилю — до {PROFILE_COMMENT_MAX_LENGTH} символов:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text="Отмена",
                callback_data=f"profile_comment_cancel|{profile_user_id}|{collection_id}"
            )]
        ])
    )
    await call.answer()
...
@router.callback_query(F.data.startswith("profile_comment_cancel|"))
async def profile_comment_cancel(call: types.CallbackQuery, state: FSMContext):
    await state.clear()
    try:
        _, profile_raw, collection_raw = call.data.split("|", 2)
        profile_user_id = int(profile_raw)
        ...
        await call.answer("Некорректные данные", show_alert=True)
        return
    await show_profile_comments(call, profile_user_id, 0, collection_id)


@router.message(ProfileCommentState.waiting_text, F.content_type == ContentType.TEXT)
async def save_profile_comment(message: types.Message, state: FSMContext):
    comment_text = (message.text or "").strip()
    if not comment_text:
        await message.answer("Комментарий не может быть пустым.")
        return
    if len(comment_text) > PROFILE_COMMENT_MAX_LENGTH:
        await message.answer(
            f"Комментарий должен быть не длиннее {PROFILE_COMMENT_MAX_LENGTH} символов."
        )
        return

    data = await state.get_data()
    profile_user_id = int(data.get("comment_profile_user_id") or 0)
    collection_id = int(data.get("comment_collection_id") or 0)
    if profile_user_id <= 0:
        await state.clear()
        await message.answer("Не удалось определить профиль для комментария.")
        return
    if message.from_user.id == profile_user_id:
        await state.clear()
        await message.answer("Нельзя оставлять комментарий самому себе.")
        return

    nickname_html, _, _ = get_user_profile(message.from_user.id)
    if nickname_html:
        author_name_html = str(nickname_html).strip()
        author_name = profile_html_to_plain(author_name_html).strip()
    else:
        author_name = (
    ...
    cursor.execute(
        ...
        INSERT INTO profile_comments (
            profile_user_id, author_user_id, author_name, author_name_html,
            comment_text, created_at, moderation_status
        ) VALUES (?, ?, ?, ?, ?, ?, 'pending')
        """,
        (
            profile_user_id,
    ...
    db.commit()
    await state.clear()
    await send_comment_for_moderation(comment_id)
    ...
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                ...
                callback_data=f"profile_comments|{profile_user_id}|0|{collection_id}"
            )]
        ])
    )


@router.message(ProfileCommentState.waiting_text)
async def profile_comment_requires_text(message: types.Message):
    await message.answer("Отправьте комментарий обычным текстовым сообщением.")




PROFILE_MODERATION_FIELD_LABELS = {
    "nickname_html": "ник",
...
async def safe_notify_user(user_id: int, text: str):
    try:
        await bot.send_message(user_id, text)
    except Exception as error:
        print(f"[MODERATION NOTIFY ERROR] user={user_id}: {error}")


async def send_comment_for_moderation(comment_id: int):
    cursor.execute(
        """
        SELECT profile_user_id, author_user_id, author_name, comment_text,
               moderation_status
        FROM profile_comments
        WHERE id=?
        """,
    ...
    profile_user_id, author_user_id, author_name, comment_text, status = row
    if status != "pending":
        return

    moderation_text = (
        "<b>Комментарий на модерацию</b>\n\n"
        "Комментарий уже опубликован.\n"
        f"Автор: <b>{html.escape(str(author_name))}</b>\n"
        f"ID автора: <code>{author_user_id}</code>\n"
        f"Профиль: <code>{profile_user_id}</code>\n\n"
        f"{html.escape(str(comment_text))}"
    )
    moderation_kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text="Одобрить",
            callback_data=f"mod_comment_approve|{comment_id}",
            style="success"
        ),
        InlineKeyboardButton(
            text="Отклонить",
            callback_data=f"mod_comment_reject|{comment_id}",
            style="danger"
        ),
    ]])
    ...
    for admin_id in ADMINS:
        try:
            ...
            print(f"[COMMENT MODERATION ERROR] admin={admin_id}: {error}")


async def submit_profile_for_moderation(
    user_id: int,
    field: str,
    submitted_value: str,
    source_user,
):
    if field not in PROFILE_MODERATION_FIELD_LABELS:
        return

    cursor.execute(
        """
        INSERT INTO profile_moderation (
            user_id, field, submitted_value, status, created_at
        ) VALUES (?, ?, ?, 'pending', ?)
        """,
        (user_id, field, submitted_value, datetime.now().isoformat())
    )
    moderation_id = cursor.lastrowid
    db.commit()

    field_label = PROFILE_MODERATION_FIELD_LABELS[field]
    telegram_name = (
        getattr(source_user, "full_name", None)
        or (f"@{source_user.username}" if getattr(source_user, "username", None) else None)
        or str(user_id)
    )
    if field in {"nickname_html", "description_html"}:
        preview = profile_html_to_plain(submitted_value).strip() or "—"
    else:
        media_type, _ = unpack_profile_media(submitted_value)
        preview = "GIF-анимация" if media_type == "animation" else "Фотография"

    moderation_text = (
        "<b>Профиль на модерацию</b>\n\n"
        "Изменение уже опубликовано.\n"
        f"Пользователь: <b>{html.escape(str(telegram_name))}</b>\n"
        f"ID: <code>{user_id}</code>\n"
        f"Поле: <b>{html.escape(field_label)}</b>\n"
        f"Значение: {html.escape(preview)}"
    )
    moderation_kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text="Одобрить",
            callback_data=f"mod_profile_approve|{moderation_id}",
            style="success"
        ),
        InlineKeyboardButton(
            text=f"Отклонить {field_label}",
            callback_data=f"mod_profile_reject|{moderation_id}",
            style="danger"
        ),
    ]])
    ...
    for admin_id in ADMINS:
        try:
            if field == "photo":
                media_type, file_id = unpack_profile_media(submitted_value)
                if media_type == "animation" and file_id:
            ...
            print(f"[PROFILE MODERATION ERROR] admin={admin_id}: {error}")
            try:
                await bot.send_message(
                    admin_id,
                    moderation_text,
                ...
                print(f"[PROFILE MODERATION FALLBACK ERROR] admin={admin_id}: {fallback_error}")


async def remove_moderation_buttons(call: types.CallbackQuery):
    try:
...
@router.callback_query(F.data.startswith("mod_comment_approve|"))
async def moderate_comment_approve(call: types.CallbackQuery):
    if not call.from_user or call.from_user.id not in ADMINS:
        await call.answer("Нет доступа", show_alert=True)
        return
    try:
        comment_id = int(call.data.rsplit("|", 1)[1])
    except (TypeError, ValueError):
        await call.answer("Некорректные данные", show_alert=True)
        return

    cursor.execute(
        "SELECT moderation_status FROM profile_comments WHERE id=?",
        (comment_id,)
    )
    row = cursor.fetchone()
    if not row or row[0] != "pending":
        await call.answer("Комментарий уже обработан", show_alert=True)
        return

    cursor.execute(
        "UPDATE profile_comments SET moderation_status='approved' WHERE id=?",
        (comment_id,)
    )
    db.commit()
    await remove_moderation_buttons(call)
    await call.answer("Комментарий одобрен")


@router.callback_query(F.data.startswith("mod_comment_reject|"))
async def moderate_comment_reject(call: types.CallbackQuery):
    if not call.from_user or call.from_user.id not in ADMINS:
        await call.answer("Нет доступа", show_alert=True)
        return
    try:
    ...
    cursor.execute(
        """
        SELECT author_user_id, moderation_status
        FROM profile_comments
        WHERE id=?
        """,
        (comment_id,)
    ...
    author_user_id, _ = row
    cursor.execute(
        "UPDATE profile_comments SET moderation_status='rejected' WHERE id=?",
        (comment_id,)
    )
    db.commit()
    await safe_notify_user(
        author_user_id,
        "Ваш комментарий был отклонён модератором и удалён из профиля."
    )
    await remove_moderation_buttons(call)
    await call.answer("Комментарий отклонён")


@router.callback_query(F.data.startswith("mod_profile_approve|"))
async def moderate_profile_approve(call: types.CallbackQuery):
    if not call.from_user or call.from_user.id not in ADMINS:
        await call.answer("Нет доступа", show_alert=True)
        return
    try:
        moderation_id = int(call.data.rsplit("|", 1)[1])
    except (TypeError, ValueError):
        await call.answer("Некорректные данные", show_alert=True)
        return

    cursor.execute(
        "SELECT status FROM profile_moderation WHERE id=?",
        (moderation_id,)
    )
    row = cursor.fetchone()
    if not row or row[0] != "pending":
        await call.answer("Изменение уже обработано", show_alert=True)
        return
    ...
    cursor.execute(
        ...
        WHERE id=?
        """,
        (call.from_user.id, datetime.now().isoformat(), moderation_id)
    )
    db.commit()
    await remove_moderation_buttons(call)
    await call.answer("Изменение профиля одобрено")


@router.callback_query(F.data.startswith("mod_profile_reject|"))
async def moderate_profile_reject(call: types.CallbackQuery):
    if not call.from_user or call.from_user.id not in ADMINS:
        await call.answer("Нет доступа", show_alert=True)
        return
    try:
    ...
    cursor.execute(
        """
        SELECT user_id, field, submitted_value, status
    ...
    user_id, field, submitted_value, _ = row
    nickname_html, description_html, photo = get_user_profile(user_id)
    current_values = {
    ...
    if (current_value or "") != (submitted_value or ""):
        cursor.execute(
            """
            UPDATE profile_moderation
        ...
        db.commit()
        await remove_moderation_buttons(call)
        await call.answer("Пользователь уже изменил это поле — старая версия не удалена", show_alert=True)
        return

    update_user_profile_field(user_id, field, None)
    cursor.execute(
        """
        UPDATE profile_moderation
    ...
    field_label = PROFILE_MODERATION_FIELD_LABELS.get(field, "поле профиля")
    await safe_notify_user(
        user_id,
        f"Ваше поле профиля «{field_label}» было отклонено модератором и удалено."
    )
    await remove_moderation_buttons(call)
    await call.answer(f"Отклонено: {field_label}")
...
async def edit_profile_field(call: types.CallbackQuery, state: FSMContext):
    ...
    prompts = {
        "photo": "Отправьте новое фото профиля или GIF. GIF доступны администраторам и владельцам подписки навсегда:",
        "nickname": (
...
async def save_profile_nickname(message: types.Message, state: FSMContext):
    ...
    await submit_profile_for_moderation(
        message.from_user.id, "nickname_html", nickname_html, message.from_user
    )
...
async def save_profile_description(message: types.Message, state: FSMContext):
    ...
    await submit_profile_for_moderation(
        message.from_user.id, "description_html", description_html, message.from_user
    )
    await delete_profile_input_messages(message, state)
...
async def save_profile_photo(message: types.Message, state: FSMContext):
    if message.animation:
        if not can_use_profile_nickname_emoji(message.from_user.id):
            await send_and_track(
                message.from_user.id,
                message.answer,
                "GIF в профиле доступны только администраторам и владельцам подписки навсегда.",
                reply_markup=profile_input_back_kb
            )
            return
        media_value = f"animation:{message.animation.file_id}"
        notice = "GIF профиля сохранена"
    elif message.photo:
        media_value = message.photo[-1].file_id
        notice = "Фото сохранено"
    else:
        await send_and_track(
            message.from_user.id,
            message.answer,
            "Отправьте фотографию или GIF-анимацию (не как обычный файл).",
            reply_markup=profile_input_back_kb
        )
        return

    update_user_profile_field(message.from_user.id, "photo", media_value)
    await submit_profile_for_moderation(
        message.from_user.id, "photo", media_value, message.from_user
    )
    await delete_profile_input_messages(message, state)
    await show_profile_edit_menu(message, state, notice)
...
async def live_search(message: types.Message, state: FSMContext):
    ...
    for anime in found_animes:
        ...
        try:
            ...
            pass
        studio = html.escape(str(info.get("studio", "—"))) if info else "—"
        ...
        text = (
            ...
            f"<tg-emoji emoji-id=\"5395444784611480792\">👍</tg-emoji> Студия: {studio}"
        )
...
def ru_to_latin(text: str | None) -> str:
    """Простая транслитерация русского названия для поиска латиницей."""
