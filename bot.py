...
async def finish_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    user_id = call.from_user.id  # 👈 ВАЖНО (автор)
    ...
    # ✂️ ограничение описания
    if description and len(description) > 500:
        description = description[:500].rstrip() + "…"

    # 💾 сохраняем подборку С АВТОРОМ
    cursor.execute(
        "INSERT INTO collections (user_id, title, description, photo, status) VALUES (?, ?, ?, ?, 'pending')",
