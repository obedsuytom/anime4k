...
async def resume_watch(call: types.CallbackQuery):
    ...
    # 🔥 отправляем ту же серию через твой handler
    fake_call = call
    fake_call.data = f"ep|{make_cb_id(anime, dub, str(season), str(episode))}|0"

    await send_video(fake_call)
