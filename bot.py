...
async def send_video(call: types.CallbackQuery):
    ...
    # Функция для авто-обрезки текста кнопки
    def short_btn(text, max_len=20):
    ...
    if prev_exists:
        nav_buttons.append(
            InlineKeyboardButton(short_btn("⬅️ Предыдущая серия"), callback_data=f"ep|{make_cb_id(anime, dub, str(season), str(ep - 1))}|{page}")
        )
    if next_exists:
        nav_buttons.append(
            InlineKeyboardButton(short_btn("➡️ Следующая серия"), callback_data=f"ep|{make_cb_id(anime, dub, str(season), str(ep + 1))}|{page}")
        )
    if nav_buttons:
        kb.row(*nav_buttons)

    # Кнопки возврата — в одном ряду
    kb.row(
        InlineKeyboardButton(short_btn("⬅️ Вернуться к сериям", max_len=20), callback_data=f"dub|{make_cb_id(anime, dub, str(season))}|{page}"),
        InlineKeyboardButton(short_btn("⬅️ Назад в меню", max_len=20), callback_data="back_menu")
    )
