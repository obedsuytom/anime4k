...
async def multi_episodes_start(
    ...
    if not has_multi_episode_access(user_id):
        await call.answer(
            "❌ Функция доступна только по подписке от 30 дней.",
            show_alert=True
        )
        return
    ...
    await state.set_state(
        MultiEpisodes.waiting_range
    )

    await call.message.answer(
        "<b>🎬 Просмотр нескольких серий</b>\n\n"
        "Введите диапазон серий.\n\n"
        "Примеры:\n"
        "<code>1-24</code>\n"
        "<code>1000-1024</code>\n\n"
        "Максимум: 24 серии за один запрос.",
        parse_mode="HTML"
    )
...
async def process_multi_range(
    ...
    if start_ep <= 0 or end_ep <= 0:
        await message.answer("❌ Номер серии должен быть больше нуля.")
        return

    if end_ep < start_ep:
        await message.answer("❌ Конечная серия не может быть меньше начальной.")
        return

    if (end_ep - start_ep + 1) > 24:
        await message.answer("❌ Максимум 24 серии за один запрос.")
        return
    ...
    if not anime:
        await message.answer("❌ Не удалось определить аниме.")
        await state.clear()
        return
    ...
    await message.answer(
        f"📤 Найдено серий: {len(episodes)}\n"
        f"Диапазон: {start_ep}-{end_ep}\n\n"
        f"Начинаю отправку..."
    )
    ...
    for ep, file_id in episodes:
        try:
            await send_episode(   # ← ВОТ ТВОЯ ВЫНЕСЕННАЯ ФУНКЦИЯ
                user_id=message.from_user.id,
                chat_id=message.chat.id,
                anime=anime,
                dub=dub,
    ...
    await message.answer(f"✅ Отправлено серий: {sent_count}")
    await state.clear()@router.message(MultiEpisodes.waiting_range)
async def process_multi_range(message: types.Message, state: FSMContext):
    ...
    # --- ОТПРАВКА ЧЕРЕЗ ТВОЙ send_episode ---
    for ep, file_id in episodes:
        try:
            msg_id = await send_episode(
                user_id=message.from_user.id,
                chat_id=message.chat.id,
                anime=anime,
                dub=dub,
            ...
            session_messages.append(msg_id)
    ...
    builder.row(
        InlineKeyboardButton(
            text="📺 К аниме",
            callback_data="multi_cleanup|anime"
        )
    )
    ...
    builder.row(
        InlineKeyboardButton(
            text="🏠 В меню",
            callback_data="multi_cleanup|menu"
        )
    )

    await message.answer(
        "⚠️ Если нажать кнопку ниже — все отправленные серии будут удалены ."
        reply_markup=builder.as_markup()
    )

    await state.clear()

@router.callback_query(lambda c: c.data.startswith("multi_cleanup"))
async def multi_cleanup(call: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    ...
    await call.answer("🗑 Сессия очищена")

    if action == "menu":
        await call.message.answer("🏠 Меню")
    else:
        await call.message.answer("📺 Аниме")
...
async def send_episode(
    ...
    # 🔥 ВАЖНО: сохраняем результат отправки
    if isinstance(file_id, str) and file_id.startswith("http"):
        msg = await send_and_track(
            user_id,
            bot.send_message,
