...
async def show_bookmark_list(call, status, page=0):
    ...
    for anime in items:
        builder.row(InlineKeyboardButton(
            text=anime[:40],
            callback_data=f"anime_bookmark|{anime}"
        ))
...
async def anime_bookmark(call: types.CallbackQuery):
    anime=call.data.split("|",1)[1]
    await show_anime_page(call.message, anime)
...
async def anime_selected(call: types.CallbackQuery):
    ...
    builder.row(
        InlineKeyboardButton(
            ...
            callback_data=f"set_status|{anime}|{next_status}"
        ),
        InlineKeyboardButton(
            text="⭐✓" if "favorite" in statuses else "⭐",
            callback_data=f"favorite|{anime}"
        )
    )
...
async def choose_dub_after_season(call: types.CallbackQuery):
    ...
    builder.row(
        InlineKeyboardButton(
            text=BOOKMARK_NAMES.get(next_status, "👀 Смотрю"),
            callback_data=f"set_status|{anime_name}|{next_status}"
        ),
        InlineKeyboardButton(
            text="⭐" if "favorite" not in statuses else "⭐✓",
            callback_data=f"favorite|{anime_name}"
        )
    )
...
async def set_status(call: types.CallbackQuery):
    _, anime, status = call.data.split("|",2)
