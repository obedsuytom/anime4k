...
async def log_all_callbacks(call: types.CallbackQuery):
    logging.info(f"[Callback] user={call.from_user.id} data={call.data}")
    await call.answer()
...
async def create_crypto_invoice_async(user_id, rub_amount, period_key):
    url = "https://cryptobot.xyz/api/create_invoice"
    headers = {"Crypto-Pay-API-Token": CRYPTOBOT_TOKEN}
    data = {
        "asset": "USDT",  # Можно поменять на RUB, BTC, ETH и т.д.
        "amount": rub_amount,
        "description": f"Оплата подписки ({period_key})",
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data, headers=headers) as resp:
                ...
                if pay_url and invoice_id:
                    ...
                    PENDING_PAYMENTS[user_id] = {"period_key": period_key, "invoice_id": invoice_id}
                    return pay_url

    except Exception as e:
        print(f"[CryptoBot] Ошибка при создании invoice: {e}")
        return None
