...
async def process_tariff(call: types.CallbackQuery):
    ...
    # Маппинг кнопок buy_… → ключ тарифа
    tariffs_map = {
        "buy_7": "7_days",
        "buy_30": "30_days",
        "buy_180": "180_days",
...
async def pay_rub_handler(call: types.CallbackQuery):
    ...
    if not pending or "period_key" not in pending:
...
def create_crypto_invoice(user_id, rub_amount, period_key):
    ...
    try:
        ...
        if pay_url and invoice_id:
            # Сохраняем invoice_id в pending
            PENDING_PAYMENTS[user_id]["invoice_id"] = invoice_id
            return pay_url
...
async def pay_crypto_handler(call: types.CallbackQuery):
    ...
    # Ожидаем callback_data вида "pay_crypto|<period_key>"
    try:
    ...
    # Отправляем сообщение и трекаем его
    await send_and_track(
        user_id,
        call.message.answer,
        text,
...
async def handle_crypto_webhook(request):
    try:
        ...
        # 🔐 2. Проверка, что invoice действительно создан
        pending = PENDING_PAYMENTS.get(user_id)
        if not pending or pending.get("invoice_id") != invoice_id:
