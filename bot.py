...
async def process_tariff(call: types.CallbackQuery):
    ...
    builder.row(InlineKeyboardButton(text="💎 Оплатить криптовалютой", callback_data=f"pay_crypto|{period_key}"))
    builder.row(InlineKeyboardButton(text="💵 Оплатить рублями", callback_data=f"pay_rub|{period_key}"))
...
async def pay_rub_handler(call: types.CallbackQuery):
    ...
    # Клавиатура — каждая кнопка в отдельной строке
    builder = InlineKeyboardBuilder()
...
async def back_to_anime(call: types.CallbackQuery):
    ...
    builder.row(InlineKeyboardButton("⬅️ Назад в меню", callback_data="back_menu"))
...
async def choose_dub_after_season(call: types.CallbackQuery):
    ...
    builder.row(InlineKeyboardButton("⬅️ К аниме", callback_data=f"anime_index|{anime_idx}"))
