...
async def broadcast(message: types.Message):
    ...
    if len(parts) < 5:
        await message.answer(
            "❌ Формат:\n"
            "/nigga <user_id|all> <photo_id> <секунды> <текст>"
        )
        return
    ...
    await message.answer(f"⏳ Отправка через {delay} сек")
...
async def vpn_menu(call: types.CallbackQuery):
    text = (
        "🍿 Смотришь аниме? Тогда попробуй Q-Tunnel VPN 🌐\n\n"
        "⚡️ С VPN видео не лагают и грузятся без зависаний\n"
        "🌏 10 регионов для выбора\n"
        "💸 Всего от 110₽/мес\n"
        "🔑 Простая настройка за пару кликов\n\n"
        "Подключиться 👇\n"
        "https://t.me/QTunnel_Bot?start=partner_6265184966"
    )

    photo_id = "AgACAgIAAxkBAAKKkmoFgSbYYut0Kqnp2WH-7XVDSnFJAAJyGmsbuXsoSFpt5mHp8KvsAQADAgADeAADOwQ"
