...
# =========================
cursor.execute("""
CREATE TABLE IF NOT EXISTS anime_registry (
...
def generate_anime_id():
    while True:
        ...
        cursor.execute("SELECT 1 FROM anime_registry WHERE anime_id=?", (value,))
        if not cursor.fetchone():
            return value


def ensure_anime_id(anime_name):
    if not anime_name:
        return None
    cursor.execute("SELECT anime_id FROM anime_registry WHERE anime=?", (anime_name,))
    row = cursor.fetchone()
    if row:
        return row[0]
    anime_id = generate_anime_id()
    cursor.execute("INSERT INTO anime_registry (anime_id, anime) VALUES (?, ?)", (anime_id, anime_name))
    db.commit()
    return anime_id


def get_anime_by_id(anime_id):
    cursor.execute("SELECT anime FROM anime_registry WHERE anime_id=?", (anime_id,))
    row = cursor.fetchone()
    return row[0] if row else None


def migrate_anime_ids():
    cursor.execute("SELECT DISTINCT anime FROM videos")
    for (anime,) in cursor.fetchall():
        ensure_anime_id(anime)
...
async def start(message: types.Message, state: FSMContext):
    ...
    if args.startswith("anime_"):
        anime_id = urllib.parse.unquote(args.replace("anime_", "", 1))
    ...
    for anime in animes:
        ...
        buttons.append([
            InlineKeyboardButton(
                ...
                callback_data=f"anime_id|{ensure_anime_id(anime)}"
            )
        ])
...
async def top_rated_menu(call: types.CallbackQuery):
    ...
    for anime, score in rows:
        ...
        cb = f"anime_id|{ensure_anime_id(anime)}"
...
async def live_search(message: types.Message, state: FSMContext):
    ...
    # В обычном чате поиск только по шестизначному ID.
    if not re.fullmatch(r"\d{6}", query):
        await message.answer("❌ Введите ID аниме из 6 цифр")
        return

    anime_by_id = get_anime_by_id(query)
    if anime_by_id:
        await show_anime_page(message, anime_by_id)
        return
        ...
        results.append(
            InlineQueryResultArticle(
                ...
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="Открыть", callback_data=f"anime_id|{ensure_anime_id(anime_name)}")]
                ])
            )
        )
...
# =========================
@router.callback_query(lambda c: c.data.startswith("anime_id|"))
async def anime_selected(call: types.CallbackQuery):
    user_id = call.from_user.id

    _, anime_id = call.data.split("|", 1)
...
async def back_to_anime(call: types.CallbackQuery):
    ...
    for i, anime in enumerate(current_animes, start=start):
        ...
        builder.row(InlineKeyboardButton(text=title, callback_data=f"anime_id|{ensure_anime_id(anime)}"))
...
async def choose_dub_after_season(call: types.CallbackQuery):
    ...
    builder.row(InlineKeyboardButton(text=" К аниме", callback_data=f"anime_id|{ensure_anime_id(anime_name)}",style="danger",icon_custom_emoji_id="5352759161945867747"))
