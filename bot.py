...
async def show_bookmark_list(call, status, page=0):
    ...
    await call.message.edit_text(
        f'{BOOKMARK_NAMES.get(status, "<tg-emoji emoji-id=\\"5438496463044752972\\">👍</tg-emoji> Избранное")}',
...
async def bookmarks_menu(call: types.CallbackQuery):
    kb=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=" Избранное", icon_custom_emoji_id="5438496463044752972", callback_data="bookmark_list|favorite|0")],
        [InlineKeyboardButton(text=" Смотрю", icon_custom_emoji_id="5210956306952758910", callback_data="bookmark_list|watching|0")],
        [InlineKeyboardButton(text=" В планах", icon_custom_emoji_id="5397782960512444700", callback_data="bookmark_list|planned|0")],
        [InlineKeyboardButton(text=" Просмотрено", style="success", icon_custom_emoji_id="5206607081334906820", callback_data="bookmark_list|completed|0")],
        [InlineKeyboardButton(text=" Брошено", style="danger", icon_custom_emoji_id="5210952531676504517", callback_data="bookmark_list|dropped|0")],
        [InlineKeyboardButton(text=" Назад", style="primary", icon_custom_emoji_id="5352759161945867747", callback_data="profile_menu")]
    ])
    await call.message.edit_text("<tg-emoji emoji-id=\"5222444124698853913\">👍</tg-emoji> Ваши закладки:", reply_markup=kb)
    await call.answer()
...
async def refresh_anime_buttons(call: types.CallbackQuery, anime: str):
    """Меняет только текст и callback у двух верхних кнопок, сохраняя оформление."""
    ...
    # Следующий статус для следующего нажатия
    if current is None:
        next_status = "watching"
    elif current == "watching":
    ...
    if markup and markup.inline_keyboard:
        ...
        if len(first_row) >= 1:
            first_row[0] = first_row[0].model_copy(
                update={
                    "text": status_text,
                    "callback_data": f"set_status|{anime_id}|{next_status}"
                }
            )

        if len(first_row) >= 2:
            first_row[1] = first_row[1].model_copy(
                update={
                    "text": favorite_text,
                    "callback_data": f"favorite|{anime_id}"
                }
