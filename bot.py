...
async def process_tariff(call: types.CallbackQuery):
    ...
    builder.add(
        InlineKeyboardButton(text="💎 Оплатить криптовалютой", callback_data=f"pay_crypto|{period_key}"),
        InlineKeyboardButton(text="💵 Оплатить рублями", callback_data=f"pay_rub|{period_key}"),
        InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="back_menu")
    )
...
async def handle_check(message: types.Message):
    ...
    builder = InlineKeyboardBuilder()
    builder.add(InlineKeyboardButton("⬅️ Назад в меню", callback_data="back_menu"))
    kb = builder.as_markup()
...
async def live_search(message: types.Message):
    ...
    for anime in found_animes:
        ...
        builder = InlineKeyboardBuilder()
        builder.row(
            InlineKeyboardButton(text="▶️ Открыть", callback_data=f"anime_index|{anime_idx}"),
            InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="back_menu")
        )
...
async def show_ranges(call: types.CallbackQuery):
    ...
    # Кнопка "Назад"
    builder.add(InlineKeyboardButton(text="⬅️ К сериям", callback_data=f"dub|{dub_hash}|0"))
...
async def anime_selected(call: types.CallbackQuery):
    ...
    for season in seasons:
        ...
        builder.add(InlineKeyboardButton(text=button_text, callback_data=f"season|{cb_id}"))

    # Кнопка выхода в меню
    builder.add(InlineKeyboardButton(text="🏠 Меню", callback_data="back_menu"))

    kb = builder.as_markup(row_width=2)  # row_width задаёт количество кнопок в ряд
...
async def back_to_anime(call: types.CallbackQuery):
    ...
    # Добавляем кнопки аниме
    for i, anime in enumerate(current_animes, start=start):
        title = anime.title()
        if len(title) > MAX_TITLE_LEN:
            title = title[:MAX_TITLE_LEN - 3] + "..."
        builder.add(InlineKeyboardButton(text=title, callback_data=f"anime_index|{i}"))
    ...
    if nav_buttons:
        builder.row(*nav_buttons)  # row с навигационными кнопками

    # Кнопка выхода в меню
    builder.add(InlineKeyboardButton("⬅️ Назад в меню", callback_data="back_menu"))

    kb = builder.as_markup(row_width=1)  # row_width=1 для списка аниме
...
async def choose_dub_after_season(call: types.CallbackQuery):
    ...
    for dub in dubs:
        ...
        builder.add(InlineKeyboardButton(text=f"▶️ {dub}", callback_data=f"dub|{cb_id}"))
    ...
    # Кнопки возврата
    builder.row(
        InlineKeyboardButton("⬅️ К аниме", callback_data=f"anime_index|{anime_idx}")
    )
    builder.row(
...
async def show_episodes_after_dub(call: types.CallbackQuery):
    ...
    # Нижние кнопки
    builder.row(InlineKeyboardButton("📂 Быстрый переход", callback_data=f"ranges|{dub_hash}"))
    builder.row(InlineKeyboardButton("⬅️ К озвучкам", callback_data=f"season|{make_cb_id(anime, str(season))}"))
    builder.row(InlineKeyboardButton("🏠 Меню", callback_data="back_menu"))
...
async def send_video(call: types.CallbackQuery):
    ...
    if isinstance(file_id, str) and file_id.startswith("http"):
        builder.row(InlineKeyboardButton("🌐 Скачать серию", url=file_id))

    # Кнопки назад
    builder.row(InlineKeyboardButton("⬅️ Вернуться к сериям", callback_data=f"dub|{make_cb_id(anime, dub, str(season))}|{page}"))
    builder.row(InlineKeyboardButton("⬅️ К озвучкам", callback_data=f"season|{make_cb_id(anime, str(season))}"))
    builder.row(InlineKeyboardButton("🏠 Меню", callback_data="back_menu"))
