...
async def send_video(call: types.CallbackQuery):
    ...
    if not has_sub:
        kb = InlineKeyboardMarkup()
        kb.row(
            InlineKeyboardButton("💳 Купить", callback_data="buy_sub"),
    ...
    # --- Навигация серии (короткие тексты) ---
    nav_row = []
    if prev_exists:
        nav_row.append(
            InlineKeyboardButton(
    ...
    if isinstance(file_id, str) and file_id.startswith("http"):
        kb.row(InlineKeyboardButton("🌐 Скачать из облака", url=file_id))
