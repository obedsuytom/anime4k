...
async def cache_posters_background(bot, admin_chat_id):
    while True:
        cursor.execute(
            "SELECT anime, poster FROM anime_info WHERE poster_file_id IS NULL AND poster IS NOT NULL"
        )
        rows = cursor.fetchall()
        for anime_name, poster_url in rows:
            try:
                msg = await bot.send_photo(chat_id=admin_chat_id, photo=poster_url, disable_notification=True)
                file_id = msg.photo[-1].file_id

                cursor.execute(
                    "UPDATE anime_info SET poster_file_id=? WHERE anime=?",
                    (file_id, anime_name)
                )
                db.commit()
                print(f"📥 Cached poster for {anime_name}")
            except Exception as e:
                print(f"❌ Ошибка кэширования {anime_name}: {e}")
...
async def load_anime_info_inline(anime_name, bot, cache_chat_id):
    """
    Берём данные из anime_info или подгружаем с Shikimori и Anilist.
    Кэшируем постеры через Telegram file_id.
    Логика постеров такая же, как в anime_index|.
    """
    # Берём english_name из videos
    ...
    # Сначала пробуем взять из anime_info
    cursor.execute(
        "SELECT poster, poster_file_id, score, genres, year FROM anime_info WHERE anime=?",
        (anime_name,)
    )
    row = cursor.fetchone()

    if row:
        poster_url, poster_file_id, score, genres, year = row

        # Если нет file_id, кэшируем
        if not poster_file_id and poster_url:
            try:
                msg = await bot.send_photo(chat_id=cache_chat_id, photo=poster_url, disable_notification=True)
                poster_file_id = msg.photo[-1].file_id
                cursor.execute(
                    "UPDATE anime_info SET poster_file_id=? WHERE anime=?",
                    (poster_file_id, anime_name)
                )
                db.commit()
            except:
                poster_file_id = None

        return anime_name, poster_url, poster_file_id, score, genres, year

    # Если нет в базе — подгружаем с Shikimori
    info = await get_anime_info(anime_name)
    if not info:
        return None

    # Постер через Anilist по english_name
    ...
    # Сохраняем в anime_info
    cursor.execute(
        "INSERT OR IGNORE INTO anime_info (anime, poster, poster_file_id, score, genres, year) VALUES (?, ?, ?, ?, ?, ?)",
        (anime_name, poster_url, poster_file_id, score, genres, year)
    )
    db.commit()
...
async def inline_search(query: types.InlineQuery):
    ...
    # Берём все аниме или по поиску
    if search_text == "all":
        cursor.execute("SELECT DISTINCT anime FROM videos ORDER BY anime")
    else:
        cursor.execute(
            "SELECT DISTINCT anime FROM videos WHERE LOWER(anime) LIKE ? ORDER BY anime",
            (f"%{search_text}%",)
        )
    rows = cursor.fetchall()
    if not rows:
    ...
    for (anime_name,) in rows:
        data = await load_anime_info_inline(anime_name, bot, cache_chat_id=ADMIN_CHAT_ID)
        if not data:
            continue

        anime_name, poster_url, poster_file_id, score, genres, year = data
        description = f"⭐ {score} | 🎭 {genres} | 📅 {year}"

        # Inline-карточка — только название с описанием
        results.append(
            InlineQueryResultArticle(
                id=str(uuid4()),
                title=anime_name,
                description=description,
                input_message_content=InputTextMessageContent(message_text=anime_name),
                thumb_url=poster_url if not poster_file_id else None,
                reply_markup=None
            )
        )
