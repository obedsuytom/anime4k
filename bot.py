...
async def show_bookmark_list(call, status, page=0):
    ...
    for anime in items:
        ...
        builder.row(InlineKeyboardButton(
            text=cut_title(anime),
            callback_data=f"anime_bookmark|{anime_id}"
        ))

    nav=[]
    if page>0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"bookmark_list|{status}|{page-1}"))
    if start+ANIME_PER_PAGE < len(animes):
        nav.append(InlineKeyboardButton(text="➡️", callback_data=f"bookmark_list|{status}|{page+1}"))
    if nav:
        builder.row(*nav)

    builder.row(InlineKeyboardButton(text="⬅️ Назад", callback_data="bookmarks_menu"))
    await call.message.edit_text(
        f"{BOOKMARK_NAMES.get(status,'❤️ Избранное')}\n\nВыберите аниме:",
...
async def bookmarks_menu(call: types.CallbackQuery):
    kb=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❤️ Избранное", callback_data="bookmark_list|favorite|0")],
        [InlineKeyboardButton(text="👀 Смотрю", callback_data="bookmark_list|watching|0")],
        [InlineKeyboardButton(text="📌 В планах", callback_data="bookmark_list|planned|0")],
        [InlineKeyboardButton(text="✅ Просмотрено", callback_data="bookmark_list|completed|0")],
        [InlineKeyboardButton(text="❌ Брошено", callback_data="bookmark_list|dropped|0")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="profile_menu")]
    ])
    await call.message.edit_text("🔖 Ваши закладки:", reply_markup=kb)
    await call.answer()
...
async def anime_selected(call: types.CallbackQuery):
    ...
    status_order = ["watching", "completed", "planned", "dropped"]
    next_status = status_order[(status_order.index(current) + 1) % len(status_order)] if current in status_order else "watching"

    builder.row(
        InlineKeyboardButton(
            text=BOOKMARK_NAMES.get(next_status, "👀 Смотрю"),
            callback_data=f"set_status|{get_or_create_anime_id(anime)}|{next_status}"
        ),
        InlineKeyboardButton(
            text="⭐✓" if "favorite" in statuses else "⭐",
            callback_data=f"favorite|{get_or_create_anime_id(anime)}"
        )
    )
...
async def choose_dub_after_season(call: types.CallbackQuery):
    ...
    # --- Статус и избранное ---
    statuses = get_bookmark_status(user_id, anime_name)
    current = next((s for s in BOOKMARK_STATUSES if s in statuses), None)
    order = ["watching", "completed", "planned", "dropped"]
    next_status = order[(order.index(current)+1) % len(order)] if current in order else "watching"

    builder.row(
        InlineKeyboardButton(
            text=BOOKMARK_NAMES.get(next_status, "👀 Смотрю"),
            callback_data=f"set_status|{get_or_create_anime_id(anime_name)}|{next_status}"
        ),
        InlineKeyboardButton(
            text="⭐" if "favorite" not in statuses else "⭐✓",
            callback_data=f"favorite|{get_or_create_anime_id(anime_name)}"
        )
    )
...
async def refresh_anime_buttons(call: types.CallbackQuery, anime: str):
    """Перерисовывает кнопки статуса и избранного в карточке аниме."""
    statuses = get_bookmark_status(call.from_user.id, anime)
    current = next((s for s in BOOKMARK_STATUSES if s in statuses), None)
    order = ["watching", "completed", "planned", "dropped"]
    next_status = order[(order.index(current) + 1) % len(order)] if current in order else "watching"

    new_buttons = InlineKeyboardBuilder()
    new_buttons.row(
        InlineKeyboardButton(
            text=BOOKMARK_NAMES.get(next_status, "👀 Смотрю"),
            callback_data=f"set_status|{get_or_create_anime_id(anime)}|{next_status}"
    ...
    for season in seasons:
        season_str=str(season).lower()
        title="🎬 Фильм" if season_str in ["film","фильм","movie"] else f"Сезон {season}"
        season_buttons.append(InlineKeyboardButton(text=title, callback_data=f"season|{make_cb_id(anime,str(season))}"))
    for i in range(0,len(season_buttons),2):
        new_buttons.row(*season_buttons[i:i+2])
    new_buttons.row(InlineKeyboardButton(text=" Меню", callback_data="back_menu"))

    try:
        if call.message.photo:
            await call.message.edit_reply_markup(reply_markup=new_buttons.as_markup())
        else:
            await call.message.edit_reply_markup(reply_markup=new_buttons.as_markup())
    except Exception as e:
        print("[refresh buttons]", e)
...
async def set_status(call: types.CallbackQuery):
    ...
    for s in BOOKMARK_STATUSES:
        cursor.execute("DELETE FROM user_bookmarks WHERE user_id=? AND anime=? AND status=?", (call.from_user.id, anime, s))
    cursor.execute("INSERT OR IGNORE INTO user_bookmarks(user_id, anime, status) VALUES(?,?,?)", (call.from_user.id, anime, status))
    db.commit()
    await refresh_anime_buttons(call, anime)
