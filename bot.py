...
async def send_video(call: types.CallbackQuery):
    ...
    # -------------------------
    # Формируем клавиатуру — ВСЁ внутри функции
    kb = InlineKeyboardMarkup(row_width=2)  # 2 кнопки в ряду
    ...
    # Функция для авто-обрезки текста кнопок
    def auto_shorten(text, max_chars):
        return text if len(text) <= max_chars else text[:max_chars-1] + "…"

    # Навигационные кнопки — в одном ряду
    nav_buttons = []
    nav_count = sum([prev_exists is not None, next_exists is not None])
    max_total_chars = 32  # примерная ширина ряда для Telegram на телефоне
    max_len_per_btn = max_total_chars // max(1, nav_count)

    if prev_exists:
        nav_buttons.append(
            InlineKeyboardButton(
                auto_shorten("⬅️ Предыдущая серия", max_len_per_btn),
    ...
    if next_exists:
        nav_buttons.append(
            InlineKeyboardButton(
                auto_shorten("➡️ Следующая серия", max_len_per_btn),
                callback_data=f"ep|{make_cb_id(anime, dub, str(season), str(ep + 1))}|{page}"
            )
        )
    if nav_buttons:
    ...
    # Кнопки возврата — на одном ряду
    kb.add(
        InlineKeyboardButton(
            auto_shorten("⬅️ Вернуться к сериям", max_total_chars // 2),
            callback_data=f"dub|{make_cb_id(anime, dub, str(season))}|{page}"
        ),
        InlineKeyboardButton(
            auto_shorten("⬅️ Назад в меню", max_total_chars // 2),
            callback_data="back_menu"
        )
    )

    # Кнопка для скачивания из облака
