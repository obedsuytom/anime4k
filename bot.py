...
async def inline_search(query: types.InlineQuery):
    ...
    offset = int(query.offset or 0)  # текущая страница

    if not search_text:
        return

    # Получаем список аниме (например, из твоей таблицы videos)
    cursor.execute("SELECT DISTINCT anime FROM videos ORDER BY anime")
    all_animes = [row[0] for row in cursor.fetchall()]
    ...
    if search_text == "all":
        ...
        filtered = [a for a in all_animes if search_text in a.lower()]

    if not filtered:
        return

    # Берём текущую страницу
    page_animes = filtered[offset: offset + PAGE_SIZE]
    ...
    for i, anime_name in enumerate(page_animes):
        ...
        if info:
            try:
                poster_url = await search_anilist_poster(info.get("english") or anime_name)
            except:
                poster_url = info.get("poster")
        poster_url = poster_url or "https://via.placeholder.com/150"

        score = info.get("score", "—") if info else "—"
        genres_data = info.get("genres", "—") if info else "—"
        genres = ", ".join(genres_data) if isinstance(genres_data, list) else genres_data
    ...
    # Следующая страница
    next_offset = str(offset + PAGE_SIZE) if offset + PAGE_SIZE < len(filtered) else None

    await query.answer(results=results, cache_time=5, is_personal=True, next_offset=next_offset)
