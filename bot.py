...
async def pick_from_inline(message: types.Message, state: FSMContext):
    ...
    if not row:
        await message.answer("❌ Аниме не найдено в базе")
        return
    ...
    if anime not in selected and len(selected) >= 20:
        await message.answer("❌ Максимум 20 аниме")
        return

    # добавление / удаление
    if anime not in selected:
        selected.append(anime)
        text = f"✅ Добавлено: {string.capwords(anime)}"
    else:
        selected.remove(anime)
        text = f"❌ Убрано: {string.capwords(anime)}"

    await state.update_data(selected=selected)

    # показываем текущий список
    result_text = "📚 <b>Выбранные аниме:</b>\n\n"
