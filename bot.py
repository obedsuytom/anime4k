...
async def account_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Аварийный fallback: оставляем видимые эмодзи, убирая только tg-emoji.
        nickname_html = strip_custom_emoji_tags(nickname_html)
        description_html = strip_custom_emoji_tags(description_html)
        update_user_profile_field(user_id, "nickname_html", nickname_html)
        if description_html:
            update_user_profile_field(user_id, "description_html", description_html)

        fallback_text = f"<b>{nickname_html or html.escape(fallback_name)}</b>"
        if description_html:
            fallback_text += f"\n\n{description_html}"
        fallback_text += (
            "\n\n<b>Аниме по группам:</b>\n"
