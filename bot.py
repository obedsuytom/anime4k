...
def open_duration_db():
    """
    Отдельное SQLite-соединение только для duration_scan.
    """
...
async def detect_video_duration(
    ...
    """
    Отправляет существующее Telegram-видео,
    получает duration и удаляет временное сообщение.
    """
    ...
    try:
        ...
        logging.info(
            "[DURATION] send_video OK: file_id=%s message_id=%s",
            file_id,
        ...
        logging.warning(
            "[DURATION] duration отсутствует: %s",
            file_id
        )
        ...
        if temporary_message is not None:
            try:
                logging.info(
                    "[DURATION] delete START: message_id=%s",
                    temporary_message.message_id,
                )
                ...
                logging.info(
                    "[DURATION] delete OK: message_id=%s",
                    temporary_message.message_id,
                )

            except TelegramRetryAfter as e:
                logging.warning(
                    "[DURATION] delete RetryAfter: %s sec",
                    e.retry_after,
                )

            except asyncio.TimeoutError:
                logging.warning(
                    "[DURATION] delete timeout: message_id=%s",
                    temporary_message.message_id,
                )
...
async def detect_video_duration_retry(
    chat_id: int,
    file_id: str,
    attempts: int = 4,
) -> int | None:
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            logging.warning(
                "[DURATION] Telegram flood control: "
                "file_id=%s attempt=%s/%s wait=%s",
                file_id,
                attempt,
                attempts,
                wait_time,
...
async def duration_scan(message: types.Message):
    ...
    logging.info(
        "[DURATION_SCAN] команда получена от user_id=%s chat_id=%s",
    ...
    try:
        ...
        # ----------------------------------------------------
        # Получаем уникальные Telegram file_id
        # ----------------------------------------------------

        logging.info(
            "[DURATION_SCAN] Получение списка видео"
        )
        ...
        if not files_to_scan:
            ...
            text = (
                "✅ У всех Telegram-видео "
                "хронометраж уже заполнен."
            )

            if link_rows:
                text += (
                    f"\n\n"
                    f"⚠️ Ссылок без хронометража: "
                    f"{link_rows}"
                )
        ...
        progress_message = await message.answer(
            ...
            f"Обработано: <b>0/{total_files}</b>",
            parse_mode="HTML",
        )
        ...
        for index, row in enumerate(
            ...
            duration = await detect_video_duration_retry(
                chat_id=message.chat.id,
                file_id=file_id,
                attempts=4,
            )
            ...
            # ------------------------------------------------
                ...
                logging.warning(
                    "[DURATION_SCAN] %s/%s FAILED file_id=%s",
                    index,
                    total_files,
                    file_id,
                )
            ...
            # ------------------------------------------------
            # Пауза между запросами Telegram
            # ------------------------------------------------
        ...
        logging.info(
            "[DURATION_SCAN] FINISHED "
            "files=%s filled=%s rows=%s failed=%s",
            total_files,
            filled_files,
            filled_rows,
            len(failed_files),
        ...
        logging.exception(
            "[DURATION_SCAN] КРИТИЧЕСКАЯ ОШИБКА: %s",
            e,
        )
        ...
        logging.info(
            "[DURATION_SCAN] FINALLY завершён"
        )
...
async def register_user(call: types.CallbackQuery):
    ...
    await send_and_track(
        ...
        "<tg-emoji emoji-id=\"5449800250032143374\">👍</tg-emoji> Тебе доступна 1 неделя бесплатного просмотра.\n\n"
        "Приятного просмотра 🍿",
        parse_mode="HTML",
        reply_markup=kb
    )
