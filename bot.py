...
def create_crypto_invoice(user_id, rub_amount, period_key):
    ...
    data = {
        "asset": "USDT",
        "amount": rub_amount / 100,  # если API ожидает в долларах
    ...
    try:
        ...
        if pay_url and invoice_id:
            PENDING_PAYMENTS[user_id] = invoice_id  # для подтверждения оплаты
            return pay_url
    except Exception as e:
        print(f"[CryptoBot] Ошибка создания invoice: {e}")
    return None
