...
async def resume_watch(call: types.CallbackQuery):
    _, anime_hash, episode = call.data.split("|")
    episode = int(episode)
    ...
    for a, d, s in rows:
        if make_cb_id(a, d, str(s)) == anime_hash:
    ...
    # 🔥 вызываем send_video напрямую, без изменения call.data
    await send_video_by_params(call, anime, dub, season, episode)
...
async def watch_history(call: types.CallbackQuery):
    ...
    for anime, dub, season, episode in rows:
        ...
        anime_title = string.capwords(anime)  # например: "one piece" -> "One Piece"
        text = f"{anime_title} — {episode} серия"

        anime_hash = make_cb_id(anime, dub, str(season))
        cb = f"resume|{anime_hash}|{episode}"
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text=" ",
                callback_data=f"watch_history|{page - 1}",
                style="primary",
                icon_custom_emoji_id="5352759161945867747"
            )
        )
    ...
    try:
        await call.message.edit_text(
            f"<b><tg-emoji emoji-id=\"5350513667144163474\">👍</tg-emoji> История просмотров (стр. {page + 1}):</b>",
            parse_mode="HTML",
            reply_markup=kb
        )
    except:
