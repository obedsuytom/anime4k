...
async def load_anime_info(anime_name):
    """Берём данные из базы, если нет — подгружаем через API и добавляем в таблицу"""
    cursor.execute("SELECT anime, poster, score, genres, year, status FROM anime_info WHERE anime=?", (anime_name,))
    row = cursor.fetchone()
    if row:
        return row

    # Если в базе нет — подгружаем через API
    info = await get_anime_info(anime_name)
    if not info:
        return None

    try:
        poster = await search_anilist_poster(anime_name)
    except:
    ...
    # Добавляем в таблицу
    cursor.execute(
        "INSERT OR IGNORE INTO anime_info (anime, poster, score, genres, year, status) VALUES (?, ?, ?, ?, ?, ?)",
        (anime_name, poster, score, genres, year, status)
    )
    db.commit()
    return (anime_name, poster, score, genres, year, status)
...
async def inline_search(query: InlineQuery):
    ...
    # -----------------------------
    # 1️⃣ Если поиск по all — берём все аниме из базы
    # -----------------------------
    if search_text == "all":
        ...
        if not all_animes:
            ...
            for anime_name, english_name in rows:
                ...
                if data:
                    all_animes.append(anime_name)

    # -----------------------------
    # 3️⃣ Берём текущую страницу для пагинации
    # -----------------------------
    ...
    results = []
    for i, anime_name in enumerate(page_animes):
        data = await load_anime_info(anime_name)
        if not data:
            continue
        anime_name, poster, score, genres, year, status = data

        poster_url = poster or "https://via.placeholder.com/150"
        description = f"⭐ {score} | 🎭 {genres} | 📅 {year}"

        kb = InlineKeyboardMarkup(row_width=1)
        kb.add(
            InlineKeyboardButton("▶️ Открыть", callback_data=f"anime_index|{offset + i}"),
            InlineKeyboardButton("⬅️ Назад", switch_inline_query_current_chat=search_text)
        )
    ...
    # -----------------------------
    # 4️⃣ Настройка next_offset для листания
    # -----------------------------
