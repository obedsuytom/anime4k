...
# ===== Вебхук Crypto.bot =====
async def handle_crypto_webhook(data: dict):
    """
    ...
    try:
        ...
        print(f"[Webhook] Некорректные данные: {e}")
        return

    if status != "confirmed":
        # Оплата не подтверждена, выходим
        return

    # Вычисляем количество дней
    if not period_key:
        print(f"[Webhook] Не указан period_key для пользователя {user_id}")
        return
    ...
    try:
        ...
        print(f"[Webhook] Подписка выдана пользователю {user_id} ({period_key})")
    except Exception as e:
        print(f"[Webhook] Не удалось отправить сообщение пользователю {user_id}: {e}")
