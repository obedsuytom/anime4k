...
async def finish_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    if mode == "create":
        cursor.execute("""
            INSERT INTO collections (user_id, title, description, photo, status)
            VALUES (?, ?, ?, ?, 'pending')
    ...
    # =========================
        ...
        """, (title, description, photo, collection_id, user_id))
    ...
    for i, anime in enumerate(selected):
        cursor.execute("""
            INSERT INTO collection_items (collection_id, anime, position)
            VALUES (?, ?, ?)
    ...
    await call.message.answer("✅ Подборка отправлена на модерацию")
    await call.answer()

# =========================
# ❌ CANCEL (универсальный)
# =========================
...
async def create_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    # 👉 режим создания
    await state.update_data(mode="create", selected=[])
...
async def set_title(message: types.Message, state: FSMContext):
    ...
    # ⚠️ не затираем selected если это edit
    selected = data.get("selected", [])
    ...
    await message.answer(
        "👍 Введи описание подборки (до 500 символов):",
        parse_mode="HTML",
        reply_markup=cancel_kb
    )
...
async def broadcast(message: types.Message):
    ...
    if len(parts) < 5:
        await message.answer(
            "❌ Формат:\n"
            "/broadcast <user_id | all> <photo_id> <секунды> <текст>"
        )
        return
...
@router.callback_query(F.data.startswith("edit_collection|"))
async def edit_collection(call: types.CallbackQuery, state: FSMContext):
    collection_id = int(call.data.split("|")[1])
    ...
    cursor.execute("""
        SELECT anime FROM collection_items
        WHERE collection_id=?
        ORDER BY position
    """, (collection_id,))
    ...
    # 🔥 ВАЖНО: удаляем старое сообщение
    try:
        await call.message.delete()
    except:
        pass

    # 👉 отправляем новое (не edit!)
...
# 🧠 EDIT MENU
# =========================
async def show_edit_menu(target, state):
    data = await state.get_data()
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="🎬 Аниме", callback_data="edit_anime")],
        [InlineKeyboardButton(text="✅ Сохранить", callback_data="save_edit")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_collection")]
    ]
...
async def edit_title(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(EditCollection.title)
    await call.message.answer("✏️ Введи новое название:")
    await call.answer()


@router.message(EditCollection.title)
async def set_edit_title(message: types.Message, state: FSMContext):
...
async def edit_description(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(EditCollection.description)
    await call.message.answer("📄 Введи описание (до 500):")
    await call.answer()


@router.message(EditCollection.description)
async def set_edit_description(message: types.Message, state: FSMContext):
    if len(message.text) > 500:
...
async def edit_photo(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(EditCollection.photo)
    await call.message.answer("🖼 Отправь новое фото:")
    await call.answer()


@router.message(EditCollection.photo)
async def set_edit_photo(message: types.Message, state: FSMContext):
    if not message.photo:
...
async def edit_anime(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(EditCollection.picking)
    await show_picker(call, state)
    await call.answer()
...
async def save_edit(call: types.CallbackQuery, state: FSMContext):
    ...
    if not title or not photo or not selected:
        await call.answer("❌ Не все данные заполнены", show_alert=True)
        return
    ...
    cursor.execute("""
        ...
        SET title=?, description=?, photo=?, status='pending'
        WHERE id=?
    """, (title, description, photo, collection_id))

    # 🧹 удаляем старые аниме
    cursor.execute(
        "DELETE FROM collection_items WHERE collection_id=?",
        (collection_id,)
    )
    ...
    await call.message.answer("✅ Подборка обновлена и отправлена на модерацию")
    await call.answer()

@router.callback_query(F.data.startswith("open_my_collection"))
async def open_my_collection(call: types.CallbackQuery):
    collection_id = int(call.data.split("|")[1])
    user_id = call.from_user.id

    cursor.execute("""
        SELECT title, description, photo, user_id
        FROM collections
        WHERE id=?
    """, (collection_id,))
    row = cursor.fetchone()
    ...
    if owner_id != user_id:
        await call.answer("Это не твоя подборка", show_alert=True)
        return
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text=" Редактировать",
                callback_data=f"edit_collection|{collection_id}",
                style="primary"
            )
        ],
        [
...
async def collections_menu(call: types.CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text=" Мои подборки",
        ...
        [
            InlineKeyboardButton(
                text=" Лайкнутые",
                callback_data="liked_collections",
                style="primary"
            )
        ],
        [
...
@router.callback_query(F.data == "liked_collections")
async def liked_collections(call: types.CallbackQuery):
    user_id = call.from_user.id

    cursor.execute("""
        SELECT c.id, c.title
        FROM collections c
        JOIN collection_likes l ON c.id = l.collection_id
        WHERE l.user_id=? AND c.status='approved'
    """, (user_id,))
    rows = cursor.fetchall()

    if not rows:
        await call.answer("Нет лайкнутых подборок", show_alert=True)
        return
    ...
    await call.message.edit_text(
        "❤️ Лайкнутые подборки:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )

@router.callback_query(F.data.startswith("edit_collection"))
async def edit_collection(call: types.CallbackQuery):
    await call.answer("Редактирование пока не сделано", show_alert=True)
...
async def my_collections(call: types.CallbackQuery):
    ...
    if not rows:
        await call.answer("У тебя нет подборок", show_alert=True)
        return
    ...
    for col_id, title in rows:
        buttons.append([
            InlineKeyboardButton(
                text=title,
                callback_data=f"open_my_collection|{col_id}"
            )
        ])
...
async def live_search(message: types.Message, state: FSMContext):
    ...
    # ❗ игнор при создании И редактировании
    if current_state in (
        CreateCollection.picking.state,
        EditCollection.picking.state
    ):
        return
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    for anime in found_animes:
        ...
        # ==============================
        # 🔥 НОВАЯ ЛОГИКА ПОСТЕРА
        # ==============================

        # 1. пробуем взять из anime_info
        cursor.execute(
        ...
        if not poster_url and info:
            ...
            # fallback — оригинальное название
            if not poster_url:
                try:
