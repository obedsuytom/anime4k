...
async def load_anime_info(anime_name, bot, cache_chat_id):
    """
    ...
    # Берём english_name для поиска постера
    ...
    if row:
        ...
        # Если нет file_id, подгружаем в Telegram и сохраняем
    ...
    # Если аниме нет в базе — подгружаем полностью
    info = await get_anime_info(anime_name)
    if not info:
        return None

    # Постер с Anilist по english_name
    try:
...
async def inline_search(query: types.InlineQuery):
    ...
    # Если поиск "all" — берём все аниме
    if search_text == "all":
        cursor.execute("SELECT DISTINCT anime FROM videos ORDER BY anime")
    else:
        cursor.execute(
    ...
    for (anime_name,) in rows:
        # Загружаем информацию и постер через кэш
        data = await load_anime_info(anime_name, bot, cache_chat_id=ADMIN_CHAT_ID)
        if not data:
            continue
        ...
        if len(results) >= 50:  # лимит на Telegram
            break
