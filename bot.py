...
async def collection_author_profile(call: types.CallbackQuery):
    ...
    description_display = description_html or "Описание профиля не указано."
    text = (
        "<b>Профиль автора</b>\n\n"
        f"Ник: {nickname_html}\n"
...
async def edit_profile_field(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await call.message.answer(prompts[field], reply_markup=profile_input_back_kb)
    await call.answer()
...
async def save_profile_nickname(message: types.Message, state: FSMContext):
    ...
    if not nickname:
        await message.answer("Ник не может быть пустым", reply_markup=profile_input_back_kb)
        return
...
async def save_profile_photo(message: types.Message, state: FSMContext):
    if not message.photo:
        await message.answer("Отправьте именно фотографию", reply_markup=profile_input_back_kb)
        return
