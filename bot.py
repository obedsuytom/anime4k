    ...
    if args.startswith("anime_"):
        ...
        if not has_active_sub(user_id):
            kb = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="💳 Купить подписку", url=payment_bot_url("pay"))],
...
async def main():
    ...
    # В основном боте платежные вебхуки больше не нужны.
    # Они переехали в отдельный payment_bot.py.
    # Если по старой причине нужно оставить их тут — поставь RUN_PAYMENT_WEBHOOKS=1.
