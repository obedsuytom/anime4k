...
r = requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/setWebhook?url={WEBHOOK_FULL_URL}")
...
# ===== Криптооплата =====
payload = f"{user_id}|{period_key}"  # для вебхука потом
...
def create_crypto_invoice(user_id, rub_amount, period_key):
    ...
    try:
        ...
        if pay_url and invoice_id:
            # Сохраняем invoice + period_key
            PENDING_PAYMENTS[user_id] = {"invoice_id": invoice_id, "period_key": period_key}
            return pay_url
...
async def pay_crypto_handler(call: types.CallbackQuery):
    ...
    if not period_key or rub_amount == 0:
        await call.answer("Ошибка тарифа", show_alert=True)
        return
