...
async def show_profile_edit_menu(target, state: FSMContext, notice: str = ""):
    ...
    # Проверяем не только ник/описание, но и все статические tg-emoji в тексте.
    text, _ = await sanitize_custom_emoji_html(text)
...
async def account_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Аварийный fallback не меняет сохранённый профиль: он очищает только
        # текущий вывод, чтобы валидный кастомный ник не потерялся из-за кнопки.
        # Убираем ВСЕ tg-emoji из уже собранного профиля, включая статические
        # эмодзи статистики, а также custom emoji у кнопок.
        fallback_text = strip_custom_emoji_tags(text)
        fallback_kb = strip_keyboard_custom_emoji(kb)
...
async def save_profile_nickname(message: types.Message, state: FSMContext):
    ...
    removed_emoji = 0
    if emoji_allowed:
        nickname_html, removed_emoji = await sanitize_custom_emoji_html(nickname_html)
...
async def save_profile_description(message: types.Message, state: FSMContext):
    ...
    description_html, removed_emoji = await sanitize_custom_emoji_html(description_html)
