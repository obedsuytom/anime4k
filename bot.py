...
async def show_picker(target, state, page):
    limit = 10
    offset = page * limit

    cursor.execute("SELECT DISTINCT anime FROM videos ORDER BY anime LIMIT ? OFFSET ?", (limit, offset))
    rows = [r[0] for r in cursor.fetchall()]

    data = await state.get_data()
    selected = data.get("selected", [])

    # Список всех аниме для индексов
    cursor.execute("SELECT DISTINCT anime FROM videos ORDER BY anime")
    ...
    buttons = []
    for anime in rows:
        anime_title = string.capwords(anime)
        if anime in selected:
            anime_title = "✅ " + anime_title

        idx = anime_map[anime]  # индекс
        buttons.append([InlineKeyboardButton(
            text=anime_title,
            callback_data=f"pick|{idx}|{page}"
        )])

    # пагинация
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"pick_page|{page-1}"))
    if len(rows) == limit:
        nav.append(InlineKeyboardButton(text="➡️", callback_data=f"pick_page|{page+1}"))
    if nav:
        buttons.append(nav)
    ...
    # действия
    buttons.append([InlineKeyboardButton(text="✅ Завершить", callback_data="finish_collection")])
    buttons.append([InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_collection")])

    kb = InlineKeyboardMarkup(inline_keyboard=buttons)

    try:
        if isinstance(target, types.CallbackQuery):
            await target.message.edit_text("📺 Выбери аниме:", reply_markup=kb)
        else:
            await target.answer("📺 Выбери аниме:", reply_markup=kb)
    except:
        pass
...
async def pick_anime(call: types.CallbackQuery, state: FSMContext):
    ...
    # лимит
    if anime not in selected and len(selected) >= 20:
        await call.answer("❌ Максимум 20 аниме", show_alert=True)
        return
...
async def create_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer("📝 Введи название подборки:")
    await call.answer()
...
async def set_title(message: types.Message, state: FSMContext):
    ...
    await message.answer("🖼 Отправь фото для подборки:")
...
async def set_photo(message: types.Message, state: FSMContext):
    ...
    await show_picker(message, state, 0)
...
async def open_collection(call: types.CallbackQuery):
    ...
    # Назад
    buttons.append([InlineKeyboardButton(text="🔙 Назад", callback_data="curated_lists")])
