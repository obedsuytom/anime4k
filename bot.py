...
def create_crypto_invoice(user_id, rub_amount, period_key):
    """
    Создаёт invoice через CryptoBot API и сохраняет его в PENDING_PAYMENTS.
    """
    ...
    data = {
        "asset": "RUB",
        "amount": rub_amount,
        "description": f"Подписка ({period_key})",
        "payload": f"{user_id}|{period_key}"  # payload для вебхука
    }

    try:
        response = requests.post(CRYPTOBOT_API_CREATE, headers=headers, json=data)
        ...
        if pay_url and invoice_id:
            # Сохраняем словарь с invoice_id и period_key
...
async def pay_crypto_handler(call: types.CallbackQuery):
    ...
    # Маппинг callback_data → ключ тарифа для RUB_PRICES
    tariffs_map = {
        "buy_7": "7_days",
        "buy_30": "30_days",
        "buy_180": "180_days",
    ...
    # Определяем period_key по кнопке
    period_key = tariffs_map.get(call.data)
    if not period_key:
        await call.answer("Ошибка тарифа", show_alert=True)
        return

    # Берём сумму в рублях
    rub_amount = RUB_PRICES.get(period_key)
    if not rub_amount:
        await call.answer("Ошибка тарифа", show_alert=True)
        return

    # Сохраняем pending payment для пользователя (invoice_id пока None)
    PENDING_PAYMENTS[user_id] = {"period_key": period_key, "invoice_id": None}

    # Удаляем старое сообщение с выбором оплаты
    await delete_bot_messages(user_id, chat_id)

    # Создаём invoice через CryptoBot API
    ...
    # Отправляем сообщение пользователю и трекаем его
    await send_and_track(
        user_id,
        call.message.answer,
        text,
...
async def handle_crypto_webhook(request):
    try:
        ...
        # 🔐 2. Проверка что этот invoice реально создан тобой
        expected_invoice_id = PENDING_PAYMENTS.get(user_id)

        if expected_invoice_id != invoice_id:
            print(f"[Webhook] Несовпадение invoice_id: {invoice_id}")
            return web.Response(text="Invoice mismatch", status=400)

        # 🔐 3. Отмечаем как обработанный
        PROCESSED_INVOICES.add(invoice_id)

        # Можно удалить из ожидающих
