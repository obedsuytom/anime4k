...
# Навигация по сериям (в одном ряду)
    nav_buttons = []
    if prev_exists:
        nav_buttons.append(
            InlineKeyboardButton(f"← {ep-1} Серия", callback_data=f"ep|{make_cb_id(anime, dub, str(season), str(ep - 1))}|{page}")
        )
    if next_exists:
        nav_buttons.append(
            InlineKeyboardButton(f"{ep+1} Серия →", callback_data=f"ep|{make_cb_id(anime, dub, str(season), str(ep + 1))}|{page}")
    ...
    # Назад к выбору озвучек и главное меню (в одном ряду)
    kb.row(
        InlineKeyboardButton("⬅️ Назад к озвучкам", callback_data=f"season|{make_cb_id(anime, str(season))}"),
