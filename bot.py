...
async def choose_plan(call: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text=" Приведи друга", callback_data="ref_menu",style="succes",icon_custom_emoji_id="5366355709850045324")],
...
async def send_video(call: types.CallbackQuery):
    ...
    if not has_sub:
        ...
        kb = builder.as_markup()
        await send_and_track(user_id, call.message.answer, "<tg-emoji emoji-id=\"5260293700088511294\">👍</tg-emoji> Для просмотра серии нужна подписка",parse_mode="HTML", reply_markup=kb)
        await call.answer("<tg-emoji emoji-id=\"5260293700088511294\">👍</tg-emoji> Подписка закончилась", show_alert=True)
        return
