...
async def cleanup_old_records():
    while True:
        try:
            ...
            # Удаляем старые обработанные invoice
            cursor.execute(
                "DELETE FROM processed_invoices WHERE created_at < ?",
                (cutoff.isoformat(),)
            )

            # Удаляем «зависшие» неоплаченные платежи старше 24 часов
            cursor.execute(
            ...
            db.commit()
            print("[Cleanup] Старая информация очищена")
        except Exception as e:
...
async def main():
    ...
    # 2. Запускаем задачу очистки каждые 24 часа
    asyncio.create_task(cleanup_old_records())  # coroutine должна быть асинхронной
