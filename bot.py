...
def get_profile_comments_count(profile_user_id: int) -> int:
    cursor.execute(
        "SELECT COUNT(*) FROM profile_comments WHERE profile_user_id=?",
        (int(profile_user_id),)
    )
    row = cursor.fetchone()
    return int(row[0] or 0)
...
async def show_profile_comments(
    ...
    if rows:
        for author_name, comment_text in rows:
            text_parts.append(
...
async def save_profile_comment(message: types.Message, state: FSMContext):
    ...
    if nickname_html:
        author_name = profile_html_to_plain(nickname_html).strip()
    else:
        author_name = (
    ...
    cursor.execute(
        ...
        INSERT INTO profile_comments (
            profile_user_id, author_user_id, author_name, comment_text, created_at
        ) VALUES (?, ?, ?, ?, ?)
        """,
        (
            profile_user_id,
