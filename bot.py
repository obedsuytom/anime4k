...
async def process_referral_bonus(user_id: int, period_key: str):
    ...
    try:
        ...
        await bot.send_message(
            ...
            "Вы получили <b>7 дней подписки</b> 🔥",
            parse_mode="HTML"
        )
...
async def referral_menu(call: types.CallbackQuery):
    ...
    text = (
        ...
        "оба получают <b>1 неделю бесплатно</b> 🎁\n"
        "• За каждого <b>5-го приглашённого</b> вы получаете "
        "<b>1 месяц бесплатно</b> 🔥\n\n"
    )
    ...
    if invited_users:
        ...
        text += "\n👤 Пока никто не выполнил условие.\n"

    text += "\nОтправьте другу ваш код и получайте бонусы! 🚀"
...
async def enter_ref_code(message: types.Message, state: FSMContext):
    ...
    if inviter_id == user_id:
        return await message.answer("❌ Нельзя использовать свой код")
    ...
    if row and row[0]:
        return await message.answer("❌ Вы уже использовали реферальный код")
        ...
        if expire_date_str == "forever":
            ...
            if expire_date > datetime.now():
                status_text = (
                    ...
                    f" Активная подписка\n"
                    f"<tg-emoji emoji-id=\"5413879192267805083\">👍</tg-emoji> До: {expire_date.strftime('%d.%m.%Y %H:%M')}"
                )
            else:
...
async def process_tariff(call: types.CallbackQuery):
    ...
    text = (
        f"<tg-emoji emoji-id=\"5395444784611480792\">👍</tg-emoji> Вы выбрали подписку: <b>{period_key.replace('_', ' ')}</b>\n\n"
    ...
    for anime in found_animes:
        ...
        if info:
            text = (
                ...
                f"<tg-emoji emoji-id=\"5449569374065152798\">👍</tg-emoji> Жанры: {info.get('genres', '—')}"
            )
...
# Кнопка назад отдельной строкой
    ...
    await send_and_track(
        user_id,
        call.message.answer,
        "<tg-emoji emoji-id=\"5231200819986047254\">👍</tg-emoji> Выберите диапазон:",
        parse_mode="HTML",
        reply_markup=kb
    )
...
async def anime_selected(call: types.CallbackQuery):
    ...
    text = (
        ...
        f"<tg-emoji emoji-id=\"5449569374065152798\">👍</tg-emoji> Жанры: {info.get('genres', 'Неизвестно')}\n"
...
# Отдельный ряд для кнопки "Меню"
    builder.row(InlineKeyboardButton(text=" Меню", callback_data="back_menu",style="success",icon_custom_emoji_id="5416041192905265756"))
...
async def show_episodes_after_dub(call: types.CallbackQuery):
    ...
    nav_buttons = []
    if start > 0:
        nav_buttons.append(InlineKeyboardButton(text=" ", callback_data=f"dub|{dub_hash}|{page-1}",icon_custom_emoji_id="5352759161945867747"))
    if end < len(episodes):
        nav_buttons.append(InlineKeyboardButton(text=" ", callback_data=f"dub|{dub_hash}|{page+1}",icon_custom_emoji_id="5355075407743826720"))
    if nav_buttons:
        builder.row(*nav_buttons)

    # --- Нижние кнопки по одной ---
    builder.row(InlineKeyboardButton(text=" Быстрый переход", callback_data=f"ranges|{dub_hash}",style="danger",icon_custom_emoji_id="5231200819986047254"))
    builder.row(InlineKeyboardButton(text=" К озвучкам", callback_data=f"season|{make_cb_id(anime, str(season))}",style="danger",icon_custom_emoji_id="5352759161945867747"))
    builder.row(InlineKeyboardButton(text=" Меню", callback_data="back_menu",style="success",icon_custom_emoji_id="5416041192905265756"))
...
async def send_video(call: types.CallbackQuery):
    ...
    if isinstance(file_id, str) and file_id.startswith("http"):
        builder.row(InlineKeyboardButton(text="🌐 Скачать серию", url=file_id))

    # Кнопки назад по отдельным строкам
    builder.row(InlineKeyboardButton(text="⬅️ Вернуться к сериям", callback_data=f"dub|{make_cb_id(anime, dub, str(season))}|{page}"))
    builder.row(InlineKeyboardButton(text="⬅️ К озвучкам", callback_data=f"season|{make_cb_id(anime, str(season))}"))
    builder.row(InlineKeyboardButton(text="🏠 Меню", callback_data="back_menu"))
