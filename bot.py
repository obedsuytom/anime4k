...
async def finish_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    if not title or not photo:
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Ошибка данных", show_alert=True,parse_mode="HTML")
        return
    if not selected:
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Выбери хотя бы одно аниме", show_alert=True,parse_mode="HTML")
        return
    ...
    await call.message.answer("<tg-emoji emoji-id=\"5350513667144163474\">👍</tg-emoji> Подборка отправлена на модерацию",parse_mode="HTML")
    await call.answer()
...
async def create_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer("<tg-emoji emoji-id=\"5373251851074415873\">👍</tg-emoji> Введи название подборки:",parse_mode="HTML")
    await call.answer()
...
async def set_title(message: types.Message, state: FSMContext):
    ...
    await message.answer("<tg-emoji emoji-id=\"5210956306952758910\">👍</tg-emoji> Отправь фото для подборки:",parse_mode="HTML")

@router.message(CreateCollection.photo)
async def set_photo(message: types.Message, state: FSMContext):
    if not message.photo:
        await message.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Отправь фото",parse_mode="HTML")
        return
...
async def open_collection(call: types.CallbackQuery):
    ...
    row = cursor.fetchone()
    if not row:
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Подборка не найдена", show_alert=True,parse_mode="HTML")
        return
    title, photo = row
