...
def get_anime_id(anime: str):
    ...
    while True:
        ...
        cursor.execute("SELECT 1 FROM anime_ids WHERE anime_id=?", (new_id,))
        if not cursor.fetchone():
            break

    cursor.execute("INSERT INTO anime_ids (anime, anime_id) VALUES (?, ?)", (anime, new_id))
    db.commit()
    return new_id

def get_anime_by_id(anime_id: str):
    cursor.execute("SELECT anime FROM anime_ids WHERE anime_id=?", (anime_id,))
    row = cursor.fetchone()
...
async def live_search(message: types.Message, state: FSMContext):
    ...
    # Поиск теперь работает только по шестизначному ID
    if not re.fullmatch(r"\d{6}", query):
        await send_and_track(
            user_id,
            message.answer,
            "❌ Поиск теперь только по ID аниме. Введите 6 цифр.",
        )
        return

    anime = get_anime_by_id(query)
    found_animes = [anime] if anime else []
...
async def inline_search(query: types.InlineQuery):
    ...
    if genre_mode:
        ...
        # Обычный inline поиск только по шестизначному ID
        if not re.fullmatch(r"\d{6}", search_text):
            await query.answer([], cache_time=1, is_personal=True)
            return

        anime = get_anime_by_id(search_text)
