...
# Добавляем колонку безопасно, не удаля�� существующие подборки.
...
async def enter_ref_code(message: types.Message, state: FSMContext):
    ...
    if row and row[0]:
        return await message.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Вы уже использовал�� реферальный код")
...
async def collection_author_profile(call: types.CallbackQuery):
    ...
    if nickname_html:
        safe_nickname, _ = await sanitize_custom_emoji_html(nickname_html)
        if safe_nickname != nickname_html:
            nickname_html = safe_nickname
            update_user_profile_field(owner_id, "nickname_html", nickname_html)
    if description_html:
        safe_description, _ = await sanitize_custom_emoji_html(description_html)
        if safe_description != description_html:
            description_html = safe_description
            update_user_profile_field(owner_id, "description_html", description_html)
    ...
    text, _ = await sanitize_sendable_custom_emoji_html(
        text,
...
CUSTOM_EMOJI_VALIDITY_CACHE = {}
CUSTOM_EMOJI_SENDABLE_CACHE = {}
...
def message_as_safe_html(message: types.Message) -> str:
    """Сохраняет Telegram-разметку и custom emoji, экранируя обычный текст."""
    html_text = getattr(message, "html_text", None)
    if callable(html_text):
        html_text = html_text()
    return html_text or html.escape(message.text or "")
...
def strip_keyboard_custom_emoji(markup: InlineKeyboardMarkup | None):
    """Возвращает ту же клавиатуру без custom emoji у кнопок."""
...
async def sanitize_custom_emoji_html(value: str | None) -> tuple[str, int]:
    """Проверяет dynamic custom emoji ID и обезвреживает невалидные теги."""
    value = value or ""
    matches = list(CUSTOM_EMOJI_TAG_RE.finditer(value))
    if not matches:
        return value, 0

    emoji_ids = list(dict.fromkeys(match.group(1) for match in matches))
    for emoji_id in emoji_ids:
        if emoji_id in CUSTOM_EMOJI_VALIDITY_CACHE:
            continue

        # Идентификатор Telegram custom emoji всегда является числовой строкой.
        if not emoji_id.isdigit():
            CUSTOM_EMOJI_VALIDITY_CACHE[emoji_id] = False
            continue
        ...
        try:
            ...
            CUSTOM_EMOJI_VALIDITY_CACHE[emoji_id] = any(
                str(getattr(sticker, "custom_emoji_id", "")) == emoji_id
                for sticker in stickers
            )
        except Exception as error:
            logging.warning("Не удалось проверить custom emoji %s: %s", emoji_id, error)
            CUSTOM_EMOJI_VALIDITY_CACHE[emoji_id] = False
    ...
    def replace_tag(match):
        ...
        if CUSTOM_EMOJI_VALIDITY_CACHE.get(emoji_id, False):
            return match.group(0)
        removed += 1
        return match.group(2)

    return CUSTOM_EMOJI_TAG_RE.sub(replace_tag, value), removed


async def sanitize_sendable_custom_emoji_html(
    value: str | None,
    chat_id: int
) -> tuple[str, int]:
    """Оставляет только те custom emoji, которые бот реально может отправить."""
    value, removed = await sanitize_custom_emoji_html(value)
    matches = list(CUSTOM_EMOJI_TAG_RE.finditer(value))
    if not matches:
    ...
    for match in matches:
        samples.setdefault(match.group(1), match.group(2) or "🙂")

    for emoji_id, visible_emoji in samples.items():
        if emoji_id in CUSTOM_EMOJI_SENDABLE_CACHE:
            continue
        ...
        try:
            ...
            CUSTOM_EMOJI_SENDABLE_CACHE[emoji_id] = True
        except Exception as error:
            logging.warning(
                "Custom emoji %s недоступен для отправки ботом: %s",
                emoji_id,
                error
            )
            CUSTOM_EMOJI_SENDABLE_CACHE[emoji_id] = False
        finally:
            if test_message:
                try:
                    await bot.delete_message(
    ...
    def replace_unsendable(match):
        nonlocal removed
        if CUSTOM_EMOJI_SENDABLE_CACHE.get(match.group(1), False):
            return match.group(0)
        removed += 1
        return match.group(2)

    return CUSTOM_EMOJI_TAG_RE.sub(replace_unsendable, value), removed
...
def get_completed_anime_hours(user_id: int) -> int:
    """Суммирует х��онометраж просмотренных серий и округляет вверх до часов."""
    cursor.execute(
        """
...
async def show_profile_edit_menu(target, state: FSMContext, notice: str = ""):
    ...
    if nickname_html:
        ...
        if safe_nickname != nickname_html:
            nickname_html = safe_nickname
            update_user_profile_field(user_id, "nickname_html", nickname_html)
    if description_html:
        safe_description, _ = await sanitize_custom_emoji_html(description_html)
        if safe_description != description_html:
            description_html = safe_description
            update_user_profile_field(user_id, "description_html", description_html)

    fallback_name = target.from_user.full_name or target.from_user.username or "Пользова��ель"
    ...
    # Проверяем не только существование ID, но и право бота отправлять эмодзи.
    target_chat_id = (
        target.message.chat.id
        if isinstance(target, types.CallbackQuery)
        else target.chat.id
    )
    text, _ = await sanitize_sendable_custom_emoji_html(text, target_chat_id)
...
async def account_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    # Старые профили тоже проверяются: некорректный ID больше не ломает меню.
    if nickname_html:
        safe_nickname, _ = await sanitize_custom_emoji_html(nickname_html)
        if safe_nickname != nickname_html:
            nickname_html = safe_nickname
    ...
    # Некоторые ID могут работать как иконки кнопок, но быть недоступными
    # внутри HTML-сообщения. Проверяем весь готовый профиль до отправки.
    text, _ = await sanitize_sendable_custom_emoji_html(
        text,
        call.message.chat.id
    )
    ...
    try:
        ...
        # Последний гарантированный fallback: обычный текст без parse_mode и
        # новая клавиатура без icon_custom_emoji_id. Telegram не получает ни
        # одной сущности custom_emoji, поэтому профиль обязательно откроется.
...
async def save_profile_nickname(message: types.Message, state: FSMContext):
    ...
    removed_emoji = 0
    if emoji_allowed:
        nickname_html, removed_emoji = await sanitize_sendable_custom_emoji_html(
            nickname_html,
            message.chat.id
        )
    ...
    notice = "Ник сохранён"
    if removed_emoji:
        notice += ". Неподдерживаемые анимированные эмодзи сохранены как обычные"
...
async def save_profile_description(message: types.Message, state: FSMContext):
    ...
    # html_text сохраняет custom_emoji entities; это разрешено всем пользователям.
    description_html = message_as_safe_html(message)
    description_html, removed_emoji = await sanitize_sendable_custom_emoji_html(
        description_html,
        message.chat.id
    )
    update_user_profile_field(message.from_user.id, "description_html", description_html)
    await delete_profile_input_messages(message, state)
    notice = "Описание сохранено"
    if removed_emoji:
        notice += ". Неподдерживаемые анимированные эмодзи сохранены как обычные"
    await show_profile_edit_menu(message, state, notice)
...
async def delete_do(call: types.CallbackQuery):
    ...
    action = _delete_action_payload(call.from_user.id, token)
    if not action:
        await call.answer("Подтверждение устарело. И��пользуй /delete заново.", show_alert=True)
        return
...
async def _down_show_dubs(call: types.CallbackQuery, payload: dict):
    ...
    if not dubs:
        await _down_send_or_edit(call, "❌ Дл�� этого сезона нет озвучек.")
        return
...
async def stars_pre_checkout_handler(pre_checkout_query: types.PreCheckoutQuery):
    ...
    if pre_checkout_query.currency != "XTR" or pre_checkout_query.total_amount != expected_amount:
        await pre_checkout_query.answer(
            ok=False,
            error_message="Сумма пла��ежа не совпадает. Создайте счёт заново."
        )
        return
...
async def inline_search(query: types.InlineQuery):
    ...
    if genre_mode:
        ...
        # Если после исправления н��чего нет — пробуем оригинальный текст.
