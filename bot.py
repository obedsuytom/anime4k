...
async def multi_episodes_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()  # 🔥 защита от старых сессий
...
async def process_multi_range(message: types.Message, state: FSMContext):
    ...
    if await state.get_state() != MultiEpisodes.waiting_range:
        return
    ...
    # 🔥 сохраняем сессию
    await state.update_data(session_messages=session_messages)
    ...
    await state.clear()  # 🔥 ВАЖНО: больше не ждём ввод
    ...
    if target == "anime":
        ...
        await call.message.answer(
            ...
            reply_markup=InlineKeyboardBuilder().row(
                InlineKeyboardButton(
                    text="📺 К аниме",
                    callback_data=f"anime_index|{anime}"
                )
            ).as_markup()
        )
...
async def send_episode(
    ...
    msg = await send_and_track(
        user_id,
        bot.send_video if not file_id.startswith("http") else bot.send_message,
        chat_id=chat_id,
        video=file_id if not file_id.startswith("http") else None,
        text=caption if file_id.startswith("http") else None,
        caption=caption if not file_id.startswith("http") else None,
        parse_mode="HTML",
        reply_markup=kb,
        protect_content=True
    )
