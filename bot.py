...
async def inline_search(query):
    ...
    if search_text == "all":
        ...
        all_animes = [row[0] for row in cursor.fetchall()]
        # Если нет в anime_info, подгружаем через videos
        if not all_animes:
            cursor.execute(
    ...
    for i, anime_name in enumerate(page_animes):
        ...
        # Кнопка «Открыть» ведёт к callback_handler anime_selected
        kb = InlineKeyboardMarkup(row_width=1)
        kb.add(
        ...
        results.append(
            InlineQueryResultArticle(
                id=str(uuid4()),
                title=anime_name,
                description=f"⭐ {score} | 🎭 {genres} | 📅 {year}",  # отображается в поиске
                input_message_content=InputTextMessageContent(
                    message_text=f"<b>{anime_name}</b>\n⭐ {score} | 🎭 {genres} | 📅 {year}",
                    parse_mode="HTML"
                ),
    ...
    await query.answer(results=results, cache_time=10, is_personal=True, next_offset=next_offset)
