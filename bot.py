...
PAYMENT_BOT_USERNAME = (os.getenv("PAYMENT_BOT_USERNAME") or "CHANGE_ME_PAYMENT_BOT").strip().lstrip("@")
# В основном боте платёжные webhook-серверы лучше не запускать, они живут в payment_bot.py
RUN_PAYMENT_WEBHOOKS = os.getenv("RUN_PAYMENT_WEBHOOKS", "0") == "1"

def payment_bot_url(payload: str = "pay") -> str:
    username = PAYMENT_BOT_USERNAME or "CHANGE_ME_PAYMENT_BOT"
...
def remember_bot_message(user_id, chat_id, message_id):
    """Запоминает любое сообщение, которое отправил бот, чтобы /start мог его удалить."""
    try:
        ...
        user_id = int(user_id) if user_id is not None else chat_id
        ...
        logging.error(f"[remember_bot_message] {e}")


def remember_bot_message_obj(user_id, result):
    """Поддерживает и одно сообщение, и список сообщений."""
    try:
        if result is None:
            return
        ...
        remember_bot_message(user_id, chat_id, message_id)
    except Exception as e:
        logging.error(f"[remember_bot_message_obj] {e}")
...
async def delete_bot_messages(user_id, chat_id):
    """Удаляет все известные сообщения бота у пользователя."""
    ...
    try:
        ...
        logging.error(f"[delete_bot_messages:select] {e}")
    ...
    try:
        ...
        logging.error(f"[delete_bot_messages:cleanup] {e}")
...
def install_bot_message_tracker():
    """
    Автоматически запоминает сообщения, отправленные через bot.send_*.
    Это ловит обычные message.answer/call.message.answer, фото, видео, документы и invoice.
    """
...
async def choose_plan(call: types.CallbackQuery):
    pay_url = payment_bot_url("pay")

    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="💳 Перейти в бот оплаты", url=pay_url)],
            [InlineKeyboardButton(text="👥 Приведи друга", callback_data="ref_menu")],
            [InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="back_menu")]
        ]
    )

    text = (
        '<tg-emoji emoji-id="5418115271267197333">👍</tg-emoji> <b>Оплата подписки</b>\n\n'
        'Оплата вынесена в отдельного Telegram-бота.\n'
        'Нажмите кнопку ниже, выберите тариф и способ оплаты там.\n\n'
        'После оплаты подписка автоматически активируется в этом основном боте.'
    )

    try:
        await send_and_track(call.from_user.id, call.message.edit_text, text, parse_mode="HTML", reply_markup=kb)
    except Exception:
        await send_and_track(call.from_user.id, call.message.answer, text, parse_mode="HTML", reply_markup=kb)
...
async def process_multi_range(message: types.Message, state: FSMContext):
    ...
    await send_and_track(
        ...
        message.answer,
        f"📤 отправляю {len(episodes)} серий"
    )
...
async def profile_menu(call: types.CallbackQuery):
    ...
    # Отправляем новое меню и обязательно запоминаем его для очистки через /start
    await send_and_track(
        user_id,
        call.message.answer,
...
async def process_tariff(call: types.CallbackQuery):
    ...
    pay_url = payment_bot_url(f"plan_{period_key}")
    ...
    text = (
        ...
        'Оплата проходит в отдельном боте. Нажмите кнопку ниже.'
    )
...
async def pay_rub_handler(call: types.CallbackQuery):
    ...
    payment_id, pay_url = await create_yookassa_payment_async(user_id, rub_amount, period_key)
    if not payment_id or not pay_url:
        await send_and_track(
            user_id,
            call.message.answer,
            "❌ Не удалось создать платёж YooKassa. Проверь YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY и логи сервера."
        )
        await call.answer()
        return

    text = (
        f"<tg-emoji emoji-id=\"5350452584119279096\">👍</tg-emoji> <b>Оплата подписки рублями</b>\n\n"
        f"Сумма: <b>{rub_amount}₽</b>\n"
        f"Тариф: <b>{period_key.replace('_', ' ')}</b>\n\n"
        "Нажмите кнопку ниже и оплатите через YooKassa.\n"
    ...
    builder.row(
        InlineKeyboardButton(
            ...
            callback_data=f"check_yookassa|{payment_id}",
            style="success",
...
async def pay_stars_handler(call: types.CallbackQuery):
    ...
    try:
        ...
        await send_and_track(
            user_id,
            call.message.answer,
            "❌ Не удалось создать счёт Telegram Stars. Проверь логи сервера."
        )
        await call.answer()
...
async def live_search(message: types.Message, state: FSMContext):
    ...
    # Умный поиск: русское название, english_name, транслит и опечатки.
    found_animes, _ = fuzzy_search_anime(query, 0, 3)
...
async def main():
    ...
    # В основном боте платёжный webhook-сервер не нужен.
    # Он запускается только если явно поставить RUN_PAYMENT_WEBHOOKS=1.
    if RUN_PAYMENT_WEBHOOKS:
        await start_webhook()
