...
async def finish_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "✅ Подборка отправлена на модерацию",
        parse_mode="HTML"
    )
...
async def edit_collection_menu(call: types.CallbackQuery, state: FSMContext):
    collection_id = int(call.data.split("|")[1])
    user_id = call.from_user.id
    ...
    for i, a in enumerate(animes, 1):
        anime_text += f"{i}. {string.capwords(a)}\n"
    ...
    # кнопки
    buttons = [
        [InlineKeyboardButton(text="✏️ Название", callback_data=f"edit_title|{collection_id}")],
        [InlineKeyboardButton(text="📄 Описание", callback_data=f"edit_description|{collection_id}")],
        [InlineKeyboardButton(text="🖼 Фото", callback_data=f"edit_photo|{collection_id}")],
        [InlineKeyboardButton(text="📺 Аниме", callback_data=f"edit_anime|{collection_id}")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="my_collections")]
    ]
...
async def edit_title(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("✏️ Введи новое название:", reply_markup=cancel_kb)
    await call.answer()
...
async def edit_description(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("📄 Введи новое описание:", reply_markup=cancel_kb)
    await call.answer()
...
async def edit_photo(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("🖼 Отправь новое фото:", reply_markup=cancel_kb)
    await call.answer()
