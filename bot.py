...
async def pick_from_inline(message: types.Message, state: FSMContext):
    ...
    if not row:
        await message.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Аниме не найдено в базе")
        return
    ...
    if anime not in selected and len(selected) >= 50
        await message.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Максимум 50 аниме")
        return
...
async def pick_anime(call: types.CallbackQuery, state: FSMContext):
    ...
    if idx >= len(all_animes):
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Аниме не найдено", show_alert=True)
        return
    ...
    # лимит
    if anime not in selected and len(selected) >= 50:
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Максимум 50 аниме", show_alert=True)
        return
...
async def send_collection_to_admins(collection_id, title, photo, selected):
    """
    Отправляет подборку админам с кнопками Одобрить/Отклонить
    """
    buttons = [
        [
            InlineKeyboardButton(
    ...
    caption = f"<b>📚 Новая подборка на модерацию:</b>\n<b>{title}</b>\n\nСписок аниме:\n"
    for anime in selected:
        caption += f"• {string.capwords(anime)}\n"
...
async def finish_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    if not title or not photo:
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Ошибка данных", show_alert=True)
        return
    if not selected:
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Выбери хотя бы одно аниме", show_alert=True)
        return

    # Сохраняем подборку
    cursor.execute(
        "INSERT INTO collections (title, photo, status) VALUES (?, ?, 'pending')",
        (title, photo)
    )
    ...
    # Отправка админам
    await send_collection_to_admins(collection_id, title, photo, selected)
    ...
    await call.message.answer("<tg-emoji emoji-id=\"5350513667144163474\">👍</tg-emoji> Подборка отправлена на модерацию")
    await call.answer()
...
async def cancel_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Создание отменено")
    await call.answer()
...
async def create_collection(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer("<tg-emoji emoji-id=\"5373251851074415873\">👍</tg-emoji> Введи название подборки:")
    await call.answer()
...
async def set_title(message: types.Message, state: FSMContext):
    ...
    await message.answer("<tg-emoji emoji-id=\"5210956306952758910\">👍</tg-emoji> Отправь фото для подборки:")

@router.message(CreateCollection.photo)
async def set_photo(message: types.Message, state: FSMContext):
    if not message.photo:
        await message.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Отправь фото")
        return
...
async def open_collection(call: types.CallbackQuery):
    ...
    cursor.execute("SELECT title, photo FROM collections WHERE id=? AND status='approved'", (collection_id,))
    row = cursor.fetchone()
    if not row:
        await call.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Подборка не найдена", show_alert=True)
        return
    title, photo = row

    # Получаем аниме в порядке
    cursor.execute("SELECT anime FROM collection_items WHERE collection_id=? ORDER BY position", (collection_id,))
    animes = [r[0] for r in cursor.fetchall()]

    # Словарь для индексов
    cursor.execute("SELECT DISTINCT anime FROM videos ORDER BY anime")
    all_animes = [r[0] for r in cursor.fetchall()]
    ...
    # Назад
    buttons.append([InlineKeyboardButton(text=" Назад", callback_data="curated_lists",style="primary",icon_custom_emoji_id="5352759161945867747")])
