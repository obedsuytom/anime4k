...
async def show_picker(target, state):
    ...
    # действия
    buttons.append([
        InlineKeyboardButton(text="✅ Завершить", callback_data="finish_collection")
    ])
    ...
    text = f"📺 Выбрано аниме: {len(selected)}/20"
...
async def chosen_inline_handler(chosen: types.ChosenInlineResult, state: FSMContext):
    current_state = await state.get_state()

    # ❗ ВАЖНО: только при создании подборки
    if current_state != CreateCollection.picking.state:
        return

    inline_id = chosen.result_id
    anime = INLINE_CACHE.get(inline_id)
    ...
    await chosen.bot.send_message(
        chosen.from_user.id,
        f"✅ Добавлено: {string.capwords(anime)}\nВсего: {len(selected)}"
    )
