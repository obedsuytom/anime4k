...
async def set_description(message: types.Message, state: FSMContext):
    if len(message.text) > 500:
        await message.answer("<tg-emoji emoji-id=\"5210952531676504517\">👍</tg-emoji> Максимум 500 символов",parse_mode="HTML")
        return
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                json={"query": query, "variables": {"search": title_en}},
        ...
        print(f"[AniList ERROR] {title_en}: {e}")
        return None
...
async def load_anime_info(anime_name: str):
    ...
    if row:
        ...
        is_shiki = poster_url and "shikimori" in poster_url

        if not poster_url or is_shiki:
            new_poster = await search_anilist_poster(english_name)

            # если AniList НЕ нашёл → пробуем Shikimori
            if not new_poster:
                new_poster = fix_shiki_poster(info.get("poster") if info else None)

            # если вообще ничего нет → оставляем старый
            if not new_poster:
                new_poster = poster_url

            # обновляем только если изменилось
            if new_poster and new_poster != poster_url:
                cursor.execute(
                    "UPDATE anime_info SET poster=? WHERE anime=?",
                    (new_poster, anime_name)
                )
                db.commit()
...
# получаем english_name сразу (если есть)
    cursor.execute(
        "SELECT english_name FROM videos WHERE anime=? LIMIT 1",
        (anime,)
    )
    r = cursor.fetchone()
    english_name = r[0] if r and r[0] else None

    is_shiki = poster_url and "shikimori" in poster_url.lower()

# если нет постера ИЛИ он шики — ищем заново
    if not poster_url or is_shiki:

    # 1) сначала английское название (самое приоритетное)
        if english_name:
            poster_url = await search_anilist_poster(english_name)

    # 2) fallback — оригинальное название
        if not poster_url:
            poster_url = await search_anilist_poster(anime)
